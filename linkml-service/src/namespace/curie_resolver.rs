...
impl CurieResolver {
    ...
    pub fn expand_curie(&self, curie: &str) -> Result<String> {
        ...
        if let Some(captures) = CURIE_REGEX.captures(curie) {
            ...
            let local = captures.get(2).unwrap().as_str();
...
mod tests {
    ...
    fn test_curie_expansion() {
        ...
        assert_eq!(
            resolver.expand_curie("ex:Person").unwrap(),
            "http://example.org/Person"
        );

        // Test URI pass-through
        assert_eq!(
            resolver.expand_curie("http://example.org/Person").unwrap(),
            "http://example.org/Person"
        );
        ...
        assert_eq!(
            resolver.expand_curie("Person").unwrap(),
            "http://example.org/Person"
        );
    }
    ...
    fn test_same_entity() {
        ...
        // Different entities
        assert!(!resolver.same_entity("ex:Person", "ex:Animal").unwrap());
    }
    ...
    fn test_namespace_context() {
        ...
        assert_eq!(
            context.resolve("local:Thing").unwrap(),
            "http://local.org/Thing"
        );

        // Global prefix still works
        assert_eq!(
            context.resolve("global:Thing").unwrap(),
