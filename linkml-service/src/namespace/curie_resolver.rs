    ...
    pub fn expand_curie(&self, curie: &str) -> Result<String> {
        ...
        if let Some(captures) = CURIE_REGEX.captures(curie) {
            let prefix = captures.get(1)
                .expect("CURIE regex has capture group 1")
                .as_str();
...
mod tests {
    ...
    fn test_curie_expansion() {
        ...
        assert_eq!(
            resolver.expand_curie("ex:Person").expect("Should expand CURIE"),
            "http://example.org/Person"
        );

        // Test URI pass-through
        assert_eq!(
            resolver.expand_curie("http://example.org/Person").expect("Should pass through URI"),
            "http://example.org/Person"
        );
        ...
        assert_eq!(
            resolver.expand_curie("Person").expect("Should expand with default prefix"),
            "http://example.org/Person"
        );
    }
    ...
    fn test_same_entity() {
        ...
        // Different entities
        assert!(!resolver.same_entity("ex:Person", "ex:Animal").expect("Should check different entities"));
    }
    ...
    fn test_namespace_context() {
        ...
        assert_eq!(
            context.resolve("local:Thing").expect("Should resolve local prefix"),
            "http://local.org/Thing"
        );

        // Global prefix still works
        assert_eq!(
            context.resolve("global:Thing").expect("Should resolve global prefix"),
