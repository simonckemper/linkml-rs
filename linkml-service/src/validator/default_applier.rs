...
mod tests {
    ...
    fn test_slot_name_default() {
        ...
        // Apply defaults
        let applier = DefaultApplier::new(&schema);
        applier.apply_defaults(&mut data, &schema).unwrap();

        // For class-specific defaults, use apply_defaults_to_object
        if let Value::Object(ref mut obj) = data {
            applier.apply_defaults_to_object(obj, "Person").unwrap();
        }
    ...
    fn test_bnode_generation() {
        ...
        if let Value::Object(ref mut obj) = data1 {
            applier.apply_defaults_to_object(obj, "Entity").unwrap();
        }
        ...
        let id1 = data1.get("id").unwrap().as_str().unwrap();
        let id2 = data2.get("id").unwrap().as_str().unwrap();
    ...
    fn test_expression_default() {
        ...
        let applier = DefaultApplier::new(&schema);
        if let Value::Object(ref mut obj) = data {
            applier.apply_defaults_to_object(obj, "Item").unwrap();
        }
