...
impl UniqueValueTracker {
    ...
    pub fn check_and_record(
        ...
        let class_values = self.seen_values
            .entry(class_name.to_string())
            .or_default();
            
        let key_values = class_values
        ...
        for (key_name, unique_key_def) in &class_def.unique_keys {
            ...
            if let Some(composite_key) = Self::create_composite_key(
                ...
                if tracker.check_and_record(
                    ...
                    let slot_values: HashMap<String, Value> = unique_key_def.unique_key_slots
                        .iter()
                        .filter_map(|slot| {
                            Self::get_slot_value(instance, slot)
                    ...
                    issues.push(
                        ...
                        .with_context("unique_key_slots", serde_json::json!(unique_key_def.unique_key_slots))
...
mod tests {
    ...
    fn test_composite_unique_key() {
        ...
        unique_keys.insert("name_email".to_string(), UniqueKeyDefinition {
            description: Some("Name and email must be unique together".to_string()),
            unique_key_slots: vec!["first_name".to_string(), "last_name".to_string(), "email".to_string()],
            consider_nulls_inequal: Some(true),
        });
        
        let class_def = ClassDefinition {
            name: "Person".to_string(),
            slots: vec!["first_name".to_string(), "last_name".to_string(), "email".to_string()],
            unique_keys,
            ..Default::default()
        };
        ...
        assert!(issues3[0].message.contains("Duplicate values for unique key 'name_email'"));
    ...
    fn test_null_handling_in_unique_keys() {
        ...
        let mut unique_keys = IndexMap::new();
        unique_keys.insert("email".to_string(), UniqueKeyDefinition {
            description: None,
        ...
        let issues2 = validator.validate_class(&instance2, &class_def, &schema, "$.persons[1]");
        assert!(issues2.is_empty(), "Null values should be considered unique when consider_nulls_inequal is true");
    }
    ...
    fn test_null_handling_inequal_false() {
        ...
        assert_eq!(issues.len(), 1, "Non-null duplicate values should still be caught");
    }
    ...
    fn test_multiple_unique_keys() {
        ...
        let mut unique_keys = IndexMap::new();
        unique_keys.insert("ssn".to_string(), UniqueKeyDefinition {
    ...
    fn test_reset_functionality() {
        ...
        let mut unique_keys = IndexMap::new();
        unique_keys.insert("id".to_string(), UniqueKeyDefinition {
