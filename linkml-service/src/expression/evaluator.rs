...
impl<'a> EvalContext<'a> {
    ...
    fn evaluate_expr(&mut self, expr: &Expression) -> Result<Value, EvaluationError> {
        ...
        let result = match expr {
            ...
            Expression::Number(n) => Ok(Value::Number(serde_json::Number::from_f64(*n).ok_or(
                EvaluationError::NumericOverflow,
            )?)),
            Expression::String(s) => {
            ...
            Expression::Variable(name) => {
                ...
                if name.contains('.') {
                    ...
                    let mut current = self.variables.get(parts[0])
                        .ok_or_else(|| EvaluationError::UndefinedVariable { name: parts[0].to_string() })?;
                    ...
                    for part in &parts[1..] {
                        match current {
                            ...
                            _ => return Err(EvaluationError::TypeError {
                                message: format!("Cannot access property '{}' on non-object", part),
                            }),
                        }
                    }
                    Ok(current.clone())
    ...
    fn evaluate_add(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
    ...
    fn evaluate_subtract(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_multiply(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_divide(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_modulo(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_equal(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
        Ok(Value::Bool(values_equal(&left_val, &right_val)))
    }
    
    fn evaluate_not_equal(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
        Ok(Value::Bool(!values_equal(&left_val, &right_val)))
    }
    
    fn evaluate_less(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_greater(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_less_or_equal(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_greater_or_equal(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
        let right_val = self.evaluate_expr(right)?;
    ...
    fn evaluate_and(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
    ...
    fn evaluate_or(&mut self, left: &Expression, right: &Expression) -> Result<Value, EvaluationError> {
        let left_val = self.evaluate_expr(left)?;
    ...
    fn evaluate_function_call(&mut self, name: &str, args: &[Expression]) -> Result<Value, EvaluationError> {
        // Evaluate arguments
