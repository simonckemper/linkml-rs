...
impl SchemaDiff {
    ...
    /// Compare two schemas
    pub fn diff(&self, schema1: &SchemaDefinition, schema2: &SchemaDefinition) -> Result<DiffResult> {
    ...
    fn compare_classes(&self, schema1: &SchemaDefinition, schema2: &SchemaDefinition, result: &mut DiffResult) -> Result<()> {
        ...
        for class_name in classes1.difference(&classes2) {
            ...
            result.breaking_changes.push(format!("Class '{}' was removed", class_name));
        }
        
        // Modified classes
        for class_name in classes1.intersection(&classes2) {
            if let (Some(class1), Some(class2)) = (schema1.classes.get(class_name), schema2.classes.get(class_name)) {
    ...
    fn compare_class(&self, name: &str, class1: &ClassDefinition, class2: &ClassDefinition) -> Result<ClassDiff> {
        ...
        if class1.is_a != class2.is_a {
            diff.changed_attributes.insert("is_a".to_string(), AttributeChange {
                old_value: class1.is_a.as_ref().map(|v| serde_json::Value::String(v.clone())),
                new_value: class2.is_a.as_ref().map(|v| serde_json::Value::String(v.clone())),
            });
        }
        
        if self.options.include_documentation && class1.description != class2.description {
            diff.changed_attributes.insert("description".to_string(), AttributeChange {
                old_value: class1.description.as_ref().map(|v| serde_json::Value::String(v.clone())),
                new_value: class2.description.as_ref().map(|v| serde_json::Value::String(v.clone())),
            });
        }
    ...
    fn compare_slots(&self, schema1: &SchemaDefinition, schema2: &SchemaDefinition, result: &mut DiffResult) -> Result<()> {
        ...
        for slot_name in slots1.difference(&slots2) {
            ...
            result.breaking_changes.push(format!("Slot '{}' was removed", slot_name));
        }
        
        // Modified slots
        for slot_name in slots1.intersection(&slots2) {
            if let (Some(slot1), Some(slot2)) = (schema1.slots.get(slot_name), schema2.slots.get(slot_name)) {
    ...
    fn compare_slot(&self, name: &str, slot1: &SlotDefinition, slot2: &SlotDefinition) -> Result<SlotDiff> {
        ...
        if slot1.range != slot2.range {
            diff.changed_attributes.insert("range".to_string(), AttributeChange {
                old_value: slot1.range.as_ref().map(|v| serde_json::Value::String(v.clone())),
                new_value: slot2.range.as_ref().map(|v| serde_json::Value::String(v.clone())),
            });
        }
        
        // Compare required
        if slot1.required != slot2.required {
            diff.changed_attributes.insert("required".to_string(), AttributeChange {
        ...
        if slot1.multivalued != slot2.multivalued {
            diff.changed_attributes.insert("multivalued".to_string(), AttributeChange {
    ...
    fn compare_types(&self, schema1: &SchemaDefinition, schema2: &SchemaDefinition, result: &mut DiffResult) -> Result<()> {
        ...
        for type_name in types1.difference(&types2) {
            ...
            result.breaking_changes.push(format!("Type '{}' was removed", type_name));
        }
        
        // Modified types
        for type_name in types1.intersection(&types2) {
            if let (Some(type1), Some(type2)) = (schema1.types.get(type_name), schema2.types.get(type_name)) {
    ...
    fn compare_type(&self, name: &str, type1: &TypeDefinition, type2: &TypeDefinition) -> Result<TypeDiff> {
    ...
    fn compare_enums(&self, schema1: &SchemaDefinition, schema2: &SchemaDefinition, result: &mut DiffResult) -> Result<()> {
        ...
        for enum_name in enums1.difference(&enums2) {
            ...
            result.breaking_changes.push(format!("Enum '{}' was removed", enum_name));
        }
        
        // Modified enums
        for enum_name in enums1.intersection(&enums2) {
            if let (Some(enum1), Some(enum2)) = (schema1.enums.get(enum_name), schema2.enums.get(enum_name)) {
    ...
    fn compare_enum(&self, name: &str, enum1: &EnumDefinition, enum2: &EnumDefinition) -> Result<EnumDiff> {
...
impl DiffResult {
    ...
    pub fn to_unified_diff(&self) -> String {
        ...
        // Classes
        if !self.added_classes.is_empty() || !self.removed_classes.is_empty() || !self.modified_classes.is_empty() {
        ...
        // Slots
        if !self.added_slots.is_empty() || !self.removed_slots.is_empty() || !self.modified_slots.is_empty() {
    ...
    pub fn to_html(&self) -> String {
        ...
        html.push_str("<ul>\n");
        html.push_str(&format!("<li>Added: {} classes, {} slots</li>\n", self.added_classes.len(), self.added_slots.len()));
        html.push_str(&format!("<li>Removed: {} classes, {} slots</li>\n", self.removed_classes.len(), self.removed_slots.len()));
        html.push_str(&format!("<li>Modified: {} classes, {} slots</li>\n", self.modified_classes.len(), self.modified_slots.len()));
        html.push_str("</ul>\n");
    ...
    pub fn to_markdown(&self) -> String {
        ...
        // Summary
        md.push_str("## Summary\n\n");
        md.push_str(&format!("- **Added**: {} classes, {} slots\n", self.added_classes.len(), self.added_slots.len()));
        md.push_str(&format!("- **Removed**: {} classes, {} slots\n", self.removed_classes.len(), self.removed_slots.len()));
        md.push_str(&format!("- **Modified**: {} classes, {} slots\n\n", self.modified_classes.len(), self.modified_slots.len()));
