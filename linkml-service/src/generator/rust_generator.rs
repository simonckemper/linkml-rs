...
impl RustGenerator {
        ...
        for slot_name in &all_slots {
            if let Some(slot) = schema.slots.get(slot_name) {
                ...
                if !slot.required.unwrap_or(false) && !slot.multivalued.unwrap_or(false) {
                    ...
                    attrs.push("#[serde(default, skip_serializing_if = \"Vec::is_empty\")]".to_string());
                }
    ...
    fn generate_impl(
        ...
        let all_slots = collect_all_slots(class, schema)?;
        let required_slots: Vec<_> = all_slots.iter()
            .filter(|slot_name| {
...
impl AsyncGenerator for RustGenerator {
    ...
    async fn generate(
        ...
        let needs_validation = schema.slots.values().any(|slot| {
            slot.required.unwrap_or(false) || slot.pattern.is_some() || 
        ...
        let slots_with_patterns: Vec<_> = schema.slots.iter()
