...
impl GoGenerator {
    ...
    fn generate_header(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        ...
        writeln!(&mut output, "// Code generated by LinkML Go Generator. DO NOT EDIT.").map_err(Self::fmt_error_to_generator_error)?;
        writeln!(&mut output).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(&mut output, "package {}", self.package_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(&mut output).map_err(Self::fmt_error_to_generator_error)?;
        
        if let Some(description) = &schema.description {
            writeln!(&mut output, "// {}", description).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(&mut output).map_err(Self::fmt_error_to_generator_error)?;
        }
    ...
    /// Generate imports
    fn generate_imports(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        let mut output = String::new();
    ...
    fn generate_types(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        ...
        for (type_name, type_def) in &schema.types {
            ...
            writeln!(&mut output, "type {} {}", go_name, base_type).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(&mut output).map_err(Self::fmt_error_to_generator_error)?;
    ...
    fn generate_enums(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        ...
        for (enum_name, enum_def) in &schema.enums {
            ...
            writeln!(&mut output, "type {} string", go_name).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(&mut output).map_err(Self::fmt_error_to_generator_error)?;
            ...
            for (i, pv) in enum_def.permissible_values.iter().enumerate() {
                ...
                if let Some(desc) = description {
                    writeln!(&mut output, "\t// {} - {}", const_name, desc).map_err(Self::fmt_error_to_generator_error)?;
                }
        ...
        for (class_name, class_def) in &schema.classes {
            if class_def.abstract_.unwrap_or(false) {
                ...
                writeln!(&mut output, "// {} defines the interface for {}", 
                    interface_name, class_name).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(&mut output, "type {} interface {{", interface_name).map_err(Self::fmt_error_to_generator_error)?;
                ...
                for (slot_name, slot_def) in slots {
                    ...
                    writeln!(&mut output, "\t{}() {}", method_name, return_type).map_err(Self::fmt_error_to_generator_error)?;
                }
                
                if self.generate_validation {
                    writeln!(&mut output, "\tValidate() error").map_err(Self::fmt_error_to_generator_error)?;
                }
    ...
    fn generate_structs(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        ...
        for (class_name, class_def) in &schema.classes {
            ...
            if let Some(description) = &class_def.description {
                writeln!(&mut output, "// {} represents {}", struct_name, description).map_err(Self::fmt_error_to_generator_error)?;
            } else {
                writeln!(&mut output, "// {} represents a {} entity", struct_name, class_name).map_err(Self::fmt_error_to_generator_error)?;
            }
            
            // Generate struct
            writeln!(&mut output, "type {} struct {{", struct_name).map_err(Self::fmt_error_to_generator_error)?;
            ...
            for (slot_name, slot_def) in slots {
                ...
                write!(&mut output, "\t{} {}", field_name, field_type).map_err(Self::fmt_error_to_generator_error)?;
                ...
                if self.generate_json_tags {
                    ...
                    write!(&mut output, " `json:\"{}{}\"`", json_name, omitempty).map_err(Self::fmt_error_to_generator_error)?;
                }
                
                // Add comment
                if let Some(description) = &slot_def.description {
                    write!(&mut output, " // {}", description).map_err(Self::fmt_error_to_generator_error)?;
                }
    ...
    fn generate_type_validation(&self, output: &mut String, type_name: &str, pattern: &str) -> GeneratorResult<()> {
        writeln!(output, "var {}Regex = regexp.MustCompile(`{}`)", 
            type_name.to_case(Case::Camel), pattern).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output).map_err(Self::fmt_error_to_generator_error)?;
        
        writeln!(output, "// Validate checks if the {} value is valid", type_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "func (t {}) Validate() error {{", type_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "\tif !{}Regex.MatchString(string(t)) {{", 
            type_name.to_case(Case::Camel)).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "\t\treturn fmt.Errorf(\"invalid {}: %s\", t)", type_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
    ...
    fn generate_enum_validation(&self, output: &mut String, enum_name: &str, values: &[linkml_core::types::PermissibleValue]) -> GeneratorResult<()> {
        writeln!(output, "// Validate checks if the {} value is valid", enum_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "func (e {}) Validate() error {{", enum_name).map_err(Self::fmt_error_to_generator_error)?;
        ...
        writeln!(output, "\t\treturn fmt.Errorf(\"invalid {}: %s\", e)", enum_name).map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output, "}}").map_err(Self::fmt_error_to_generator_error)?;
        writeln!(output).map_err(Self::fmt_error_to_generator_error)?;
        Ok(())
        ...
        for (slot_name, slot_def) in slots {
            ...
            if slot_def.required.unwrap_or(false) {
                match slot_def.range.as_deref() {
                    Some("string") => {
                        writeln!(output, "\tif s.{} == \"\" {{", field_name).map_err(Self::fmt_error_to_generator_error)?;
                        writeln!(output, "\t\treturn fmt.Errorf(\"{} is required\")", slot_name).map_err(Self::fmt_error_to_generator_error)?;
                        writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
                    }
            ...
            if let Some(pattern) = &slot_def.pattern {
                ...
                writeln!(output, "\t\tmatched, _ := regexp.MatchString(`{}`, s.{})", 
                    pattern, field_name).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t\tif !matched {{").map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t\t\treturn fmt.Errorf(\"{} does not match pattern\")", slot_name).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t\t}}").map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
            }
            ...
            if let Some(min) = &slot_def.minimum_value {
                writeln!(output, "\tif s.{} < {} {{", field_name, min).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t\treturn fmt.Errorf(\"{} must be >= {}\")", slot_name, min).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
            }
            
            if let Some(max) = &slot_def.maximum_value {
                writeln!(output, "\tif s.{} > {} {{", field_name, max).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t\treturn fmt.Errorf(\"{} must be <= {}\")", slot_name, max).map_err(Self::fmt_error_to_generator_error)?;
                writeln!(output, "\t}}").map_err(Self::fmt_error_to_generator_error)?;
            }
    ...
    fn generate_interface_methods(&self,
        ...
        for (slot_name, slot_def) in slots {
            ...
            writeln!(output, "// {} returns the {} value", method_name, slot_name).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(output, "func (s *{}) {}() {} {{", struct_name, method_name, return_type).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(output, "\treturn s.{}", field_name).map_err(Self::fmt_error_to_generator_error)?;
            writeln!(output, "}}").map_err(Self::fmt_error_to_generator_error)?;
            writeln!(output).map_err(Self::fmt_error_to_generator_error)?;
        }
        Ok(())
