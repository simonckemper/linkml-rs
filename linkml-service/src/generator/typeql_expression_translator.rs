...
impl TranslationContext {
    ...
    #[must_use] pub fn new(entity_var: String) -> Self {
        Self {
...
type FunctionHandler = Box<dyn Fn(&[Expression], &mut TranslationContext) -> Result<TranslatedExpression, TranslationError> + Send + Sync>;
...
impl ExpressionTranslator {
    ...
    fn register_builtin_functions(&mut self) {
        // len() function
        self.function_handlers.insert("len".to_string(), Box::new(|args, _ctx| {
            if args.len() != 1 {
                return Err(TranslationError::UnsupportedExpression("len() requires exactly 1 argument".to_string()));
            }
            
            // For collections, we need to count in TypeQL
            // This is complex and may require a computed attribute
            Err(TranslationError::ComplexExpression("len() requires computed attribute in TypeQL".to_string()))
        }));
        
        // contains() function - can be translated to pattern matching
        self.function_handlers.insert("contains".to_string(), Box::new(|args, ctx| {
            if args.len() != 2 {
                return Err(TranslationError::UnsupportedExpression("contains() requires exactly 2 arguments".to_string()));
            }
            ...
            match (&args[0], &args[1]) {
                ...
                _ => Err(TranslationError::ComplexExpression("contains() with complex arguments".to_string()))
            }
        }));
    }
    ...
    pub fn translate(
        ...
        match expr {
            ...
            Expression::Subtract(left, right) => self.translate_binary_op(left, "Subtract", right, ctx),
            Expression::Multiply(left, right) => self.translate_binary_op(left, "Multiply", right, ctx),
            ...
            Expression::NotEqual(left, right) => self.translate_binary_op(left, "NotEqual", right, ctx),
            Expression::Less(left, right) => self.translate_binary_op(left, "Less", right, ctx),
            Expression::LessOrEqual(left, right) => self.translate_binary_op(left, "LessEqual", right, ctx),
            Expression::Greater(left, right) => self.translate_binary_op(left, "Greater", right, ctx),
            Expression::GreaterOrEqual(left, right) => self.translate_binary_op(left, "GreaterEqual", right, ctx),
    ...
    fn translate_literal_null(&self) -> Result<TranslatedExpression, TranslationError> {
        Err(TranslationError::UnsupportedExpression("null values in TypeQL rules".to_string()))
    }
    ...
    fn translate_binary_op(
        ...
        match op {
            ...
            "NotEqual" => {
                patterns.push(format!("not {{ {} = {}; }}", left_trans.result, right_trans.result));
            }
            "Less" => {
                patterns.push(format!("{} < {}", left_trans.result, right_trans.result));
            }
            ...
            "Add" | "Subtract" | "Multiply" | "Divide" | "Modulo" => {
                return Err(TranslationError::ComplexExpression(
                    format!("Arithmetic operation {op} requires computed attribute")
                ));
            }
            ...
            "Or" => {
                ...
                return Err(TranslationError::ComplexExpression("OR requires multiple rules".to_string()));
            }
            
            _ => {
                return Err(TranslationError::UnsupportedExpression(format!("Unknown operator: {op}")));
            }
        }
    ...
    fn translate_unary_op(
        ...
        match op {
            ...
            _ => {
                Err(TranslationError::UnsupportedExpression(format!("Unary operator {op} not supported")))
            }
        }
    }
    ...
    fn translate_function(
        ...
        if let Some(handler) = self.function_handlers.get(name) {
            ...
            Err(TranslationError::ComplexExpression("case() expressions generate multiple rules".to_string()))
        } else {
    ...
    fn translate_conditional(
        ...
        Err(TranslationError::ComplexExpression("Conditional expressions require multiple rules".to_string()))
    }
