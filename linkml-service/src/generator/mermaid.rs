...
impl MermaidGenerator {
        ...
        for (name, class_def) in &schema.classes {
            ...
            for slot_name in &all_slots {
                if let Some(slot_def) = schema.slots.get(slot_name) {
                    ...
                    let key_marker = if slot_def.identifier == Some(true) { "PK" } else { "" };
                    let required_marker = if slot_def.required == Some(true) && key_marker.is_empty() { "*" } else { "" };
    ...
    fn generate_class_diagram(&self, schema: &SchemaDefinition) -> GeneratorResult<String> {
        ...
        for (name, class_def) in &schema.classes {
            ...
            for slot_name in &all_slots {
                if let Some(slot_def) = schema.slots.get(slot_name) {
                    let visibility = if slot_def.required == Some(true) { "+" } else { "-" };
    ...
    fn generate_flowchart_children(&self, output: &mut String, parent: &str, children: &HashMap<String, Vec<String>>) -> GeneratorResult<()> {
    ...
    fn get_er_cardinality(&self, slot: &SlotDefinition) -> &'static str {
        match (slot.required.unwrap_or(false), slot.multivalued.unwrap_or(false)) {
...
impl Generator for MermaidGenerator {
    ...
    fn description(&self) -> &str {
        match self.options.diagram_type {
            MermaidDiagramType::EntityRelationship => "Generates Mermaid Entity Relationship diagrams from LinkML schemas",
            MermaidDiagramType::ClassDiagram => "Generates Mermaid class diagrams from LinkML schemas",
            MermaidDiagramType::StateDiagram => "Generates Mermaid state diagrams from LinkML schemas",
...
mod tests {
    ...
    fn test_sanitize_name() {
        ...
        assert_eq!(generator.sanitize_name("Class.With.Dots"), "Class_With_Dots");
    }
