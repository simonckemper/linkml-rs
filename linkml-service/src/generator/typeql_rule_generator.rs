...
impl TypeQLRule {
    ...
    pub fn to_typeql(&self) -> GeneratorResult<String> {
        ...
        if let Some(desc) = &self.description {
            writeln!(&mut output, "# {desc}").map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
        }
        
        // Rule definition
        writeln!(&mut output, "rule {}:", self.name).map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
        writeln!(&mut output, "when {{").map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
        
        // When patterns
        for (i, pattern) in self.when_patterns.iter().enumerate() {
            write!(&mut output, "    {pattern}").map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
            if i < self.when_patterns.len() - 1 {
                writeln!(&mut output, ";").map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
            } else {
                writeln!(&mut output).map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
            }
        }
        
        writeln!(&mut output, "}} then {{").map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
...
impl RuleGenerator {
    ...
    pub fn generate_class_rules(
        ...
        for slot_name in &class.slots {
            if let Some(slot) = schema.slots.get(slot_name)
                ...
                if let Some(expr_str) = &slot.equals_expression {
                    if let Some(rule) = self.generate_expression_rule(class_name, slot_name, expr_str) {
                        rules.push(rule);
                    }
                }
            }
        ...
        if let Some(if_required) = &class.if_required {
            for (i, (slot_name, conditional)) in if_required.iter().enumerate() {
                if let Some(rule) = self.generate_conditional_rule(class_name, slot_name, conditional, i) {
                    rules.push(rule);
                }
            }
        }
    ...
    fn generate_required_rule(&mut self, class_name: &str, slot_name: &str) -> Option<TypeQLRule> {
        let rule_name = self.generate_rule_name(&format!("{}-requires-{}", 
        ...
        let then_patterns = vec![
            format!("$x has validation-error \"Missing required field: {}\"", slot_name),
        ];
    ...
    fn generate_pattern_rule(&mut self, class_name: &str, slot_name: &str, pattern: &str) -> Option<TypeQLRule> {
        // Note: Pattern validation is handled by attribute regex in TypeQL
        // This generates a rule for invalid patterns
        let rule_name = self.generate_rule_name(&format!("{}-{}-pattern", 
            class_name.to_lowercase(), 
            slot_name.to_lowercase()
        ));
        ...
        let then_patterns = vec![
            format!("$x has validation-error \"Field {} does not match pattern: {}\"", slot_name, pattern),
        ];
    ...
    fn generate_range_rule(&mut self, class_name: &str, slot_name: &str, slot: &SlotDefinition) -> Option<TypeQLRule> {
        let rule_name = self.generate_rule_name(&format!("{}-{}-range", 
            class_name.to_lowercase(), 
            slot_name.to_lowercase()
        ));
        ...
        let then_patterns = vec![
            format!("$x has validation-error \"Field {} must be {}\"", slot_name, range_desc.join(" and ")),
        ];
    ...
    fn generate_expression_rule(&mut self, class_name: &str, slot_name: &str, expr_str: &str) -> Option<TypeQLRule> {
        ...
        match parser.parse(expr_str) {
            Ok(expr) => {
                ...
                match self.expression_translator.translate(&expr, &mut ctx) {
                    Ok(trans) => {
                        let rule_name = self.generate_rule_name(&format!("{}-compute-{}", 
                            class_name.to_lowercase(), 
                            slot_name.to_lowercase()
                        ));
                        ...
                        let then_patterns = vec![
                            format!("$x has {} {}", slot_name, trans.result),
                        ];
    ...
    fn generate_conditional_rule(
        ...
        let rule_name = self.generate_rule_name(&format!("{}-conditional-{}", 
            class_name.to_lowercase(), 
            index
        ));
        ...
        if let Some(then_required) = &conditional.then_required {
            ...
            let condition_value = conditional.condition.as_ref()
                .and_then(|c| c.equals_string.as_ref())
                .unwrap_or(&default_value);
            
            let then_patterns = vec![
                format!("$x has validation-error \"When {} is {}, the following fields are required: {}\"",
                    slot_name,
                    condition_value,
                    required_fields
                ),
    ...
    fn generate_from_linkml_rule(&mut self, class_name: &str, rule: &Rule) -> Option<TypeQLRule> {
        ...
        let rule_id = rule.title.as_ref()
            .or(rule.description.as_ref()).map_or_else(|| "rule".to_string(), |s| s.chars().take(20).collect::<String>());
        ...
        Some(TypeQLRule {
            ...
            then_patterns: vec![format!("# TODO: Implement rule from: {}", 
                rule.description.as_ref().unwrap_or(rule.title.as_ref().unwrap_or(&"unspecified".to_string())))],
    ...
    #[must_use] pub fn get_rules(&self) -> &[TypeQLRule] {
        &self.rules
    }
        ...
        if !self.rules.is_empty() {
            ...
            if !validation_rules.is_empty() {
                ...
                for rule in validation_rules {
                    ...
                    writeln!(&mut output, "{}", typeql).map_err(|e| GeneratorError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
                }
            }
...
mod tests {
    ...
    fn test_required_rule_generation() {
        ...
        let rule = generator.generate_required_rule("Person", "name").expect("should generate required rule");
        
        assert_eq!(rule.rule_type, RuleType::Validation);
        assert!(rule.when_patterns.contains(&"$x isa person".to_string()));
        assert!(rule.when_patterns.contains(&"not { $x has name $v; }".to_string()));
    ...
    fn test_range_rule_generation() {
        ...
        let rule = generator.generate_range_rule("Person", "age", &slot).expect("should generate range rule");
    ...
    fn test_rule_typeql_generation() {
        let rule = TypeQLRule {
            ...
            then_patterns: vec![
                "$x has validation-error \"Missing name\"".to_string(),
            ],
