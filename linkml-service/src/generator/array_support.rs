...
impl ArrayCodeGenerator for PythonArrayGenerator {
    ...
    fn generate_array_init(&self, spec: &ArraySpec, var_name: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        if let Some(shape) = spec.fixed_shape() {
            ...
            Self::write_or_default(
                &mut code,
                format_args!("{var_name} = np.array([], dtype={dtype})\n")
            );
        }
    ...
    fn generate_array_validation(&self, spec: &ArraySpec, var_name: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        Self::write_or_default(
            &mut code,
            format_args!("def validate_{var_name}_array(arr):\n")
        );
        Self::write_or_default(
            &mut code,
            format_args!("    \"\"\"Validate array shape and type\"\"\"\n")
        );

        // Check number of dimensions
        Self::write_or_default(
            &mut code,
            format_args!("    if arr.ndim != {}:\n", spec.ndim())
        );
        Self::write_or_default(
            &mut code,
            format_args!(
        ...
        for (i, dim) in spec.dimensions.iter().enumerate() {
            ...
            if let Some(size) = dim.size {
                Self::write_or_default(
                    &mut code,
                    format_args!("    if arr.shape[{i}] != {size}:\n")
                );
                Self::write_or_default(
                    &mut code,
                    format_args!("        raise ValueError(f\"Dimension '{}' expected size {}, got {{arr.shape[{}]}}\")\n",
                        dim.name, size, i)
                );
            }
            ...
            if let Some(min) = dim.min_size {
                Self::write_or_default(
                    &mut code,
                    format_args!("    if arr.shape[{i}] < {min}:\n")
                );
                Self::write_or_default(
                    &mut code,
                    format_args!("        raise ValueError(f\"Dimension '{}' minimum size is {}, got {{arr.shape[{}]}}\")\n",
                        dim.name, min, i)
                );
            }
            ...
            if let Some(max) = dim.max_size {
                Self::write_or_default(
                    &mut code,
                    format_args!("    if arr.shape[{i}] > {max}:\n")
                );
                Self::write_or_default(
                    &mut code,
                    format_args!("        raise ValueError(f\"Dimension '{}' maximum size is {}, got {{arr.shape[{}]}}\")\n",
                        dim.name, max, i)
                );
            }
        }
    ...
    fn generate_array_accessor(&self, spec: &ArraySpec, method_name: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        Self::write_or_default(&mut code, format_args!("    \"\"\"Get array element\"\"\"\n"));
        Self::write_or_default(
            &mut code,
            format_args!("    return self._array[{index_args}]\n")
        );

        Self::write_or_default(&mut code, format_args!("\n"));
        Self::write_or_default(
            &mut code,
            format_args!(
                "def set_{}(self, {}, value: {}):\n",
        ...
        Self::write_or_default(&mut code, format_args!("    \"\"\"Set array element\"\"\"\n"));
        Self::write_or_default(
            &mut code,
            format_args!("    self._array[{index_args}] = value\n")
        );
...
#[must_use] pub fn get_array_generator(language: &str) -> Option<Box<dyn ArrayCodeGenerator>> {
    match language {
