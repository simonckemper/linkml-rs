...
impl IcebergIntegration {
    ...
    pub fn linkml_to_iceberg(&mut self, schema: &SchemaDefinition, class_name: &str) -> Result<IcebergTableSchema> {
        ...
        properties.insert("auto_compaction".to_string(), self.config.auto_compaction.to_string());
    ...
    fn determine_partition_spec(class_def: &ClassDefinition, schema: &SchemaDefinition) -> Option<String> {
        ...
        for slot_name in &class_def.slots {
            if let Some(slot_def) = schema.slots.get(slot_name) {
                ...
                if let Some(ref range) = slot_def.range && (range == "date" || range == "datetime") {
    ...
    fn determine_sort_order(class_def: &ClassDefinition, schema: &SchemaDefinition) -> Option<String> {
        ...
        for slot_name in &class_def.slots {
            if let Some(slot_def) = schema.slots.get(slot_name) {
                ...
                else if let Some(ref range) = slot_def.range && (range == "date" || range == "datetime") {
    ...
    pub fn insert_data(
        ...
        let _iceberg_schema = self.schema_cache
            .get(&cache_key)
            .ok_or_else(|| LinkMLError::schema_validation(format!("Schema '{cache_key}' not found in cache")))?;

        println!("✓ Inserted {} records into Iceberg table for class '{}'",
            data.len(), class_name);
    ...
    pub fn query_data(
        ...
        println!("✓ Queried {} records from Iceberg table for class '{}'{}",
            results.len(), class_name,
            filters.map(|f| format!(" with filters: {f}")).unwrap_or_default());
...
#[must_use]
pub fn create_iceberg_integration(
