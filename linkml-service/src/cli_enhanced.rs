...
#[arg (short ,
     long ,
     global = true)] verbose : bool ,

#[doc = " Output format"]
#[arg (short = 'f' ,
     long ,
     global = true ,
     default_value = "pretty")] format : OutputFormat ,
...
#[doc = " Human-readable output"] Pretty ,

#[doc = " JSON output"] Json ,

#[doc = " YAML output"] Yaml ,
...
#[doc = " Validate data against a schema"] Validate  {

#[doc = " Schema file path"]
#[arg (short ,
     long)] schema : PathBuf ,

#[doc = " Data file(s) to validate"]
#[arg (required = true)] data : Vec < PathBuf > ,

#[doc = " Target class name"]
#[arg (short = 'C' ,
     long)] class_name : Option < String > ,

#[doc = " Enable strict mode"]
#[arg (long)] _strict : bool ,

#[doc = " Maximum errors to show"]
#[arg (long ,
     default_value = "10")] max_errors : usize ,

#[doc = " Show validation statistics"]
#[arg (long)] stats : bool ,
...
#[doc = " Output directory or file"]
#[arg (short ,
     long)] output : PathBuf ,

#[doc = " Generator name (python,
     typescript,
     rust,
     etc.)"]
#[arg (short = 'g' ,
     long)] generator : String ,

#[doc = " Generator options (key=value)"]
#[arg (long = "option" ,
     value_name = "KEY=VALUE")] options : Vec < String > ,

#[doc = " Template directory for custom templates"]
#[arg (short = 't' ,
...
#[doc = " Input format (auto-detect
    if not specified)"]
#[arg (long)] from : Option < SchemaFormat > ,

#[doc = " Output format"]
#[arg (long ,
     value_enum)] to : SchemaFormat ,
...
#[doc = " Schema files to merge"]
#[arg (required = true ,
     num_args = 2 ..)] schemas : Vec < PathBuf > ,
...
#[arg (short = 's' ,
     long ,
     default_value = "union")] strategy : MergeStrategy ,

#[doc = " Conflict resolution"]
#[arg (short = 'c' ,
     long ,
     default_value = "error")] conflict : ConflictResolution ,

#[doc = " Base schema for three-way merge"]
#[arg (short = 'b' ,
...
#[doc = " First schema file"] schema1 : PathBuf ,

#[doc = " Second schema file"] schema2 : PathBuf ,

#[doc = " Output format for diff"]
#[arg (short = 'f' ,
     long ,
     default_value = "unified")] format : DiffFormat ,

#[doc = " Include documentation changes"]
#[arg (long)] include_docs : bool ,

#[doc = " Show only breaking changes"]
#[arg (long)] breaking_only : bool ,

#[doc = " Context lines for unified diff"]
#[arg (short = 'c' ,
     long ,
     default_value = "3")] context : usize ,
...
#[doc = " Schema file to lint"] schema : PathBuf ,

#[doc = " Lint rules to apply"]
#[arg (short = 'r' ,
     long)] rules : Vec < String > ,

#[doc = " Configuration file for lint rules"]
#[arg (short = 'c' ,
     long)] config : Option < PathBuf > ,

#[doc = " Fix issues automatically
where possible"]
#[arg (long)] fix : bool ,

#[doc = " Fail with non-zero exit code
    if issues found"]
...
#[arg (short ,
     long ,
     default_value = "8080")] port : u16 ,

#[doc = " Host to bind to"]
#[arg (short = 'H' ,
     long ,
     default_value = "127.0.0.1")] host : String ,
...
#[arg (short = 'a' ,
     long)] auth : Option < AuthType > ,

#[doc = " TLS certificate file"]
...
#[doc = " Input format"]
#[arg (short = 'f' ,
     long)] format : LoadFormat ,

#[doc = " Output file"]
#[arg (short ,
     long)] output : PathBuf ,

#[doc = " Loader options (key=value)"]
#[arg (long = "option" ,
     value_name = "KEY=VALUE")] options : Vec < String > ,

#[doc = " Validate loaded data"]
#[arg (long)] validate : bool ,
...
#[doc = " Output format"]
#[arg (short = 'f' ,
     long)] format : DumpFormat ,

#[doc = " Dumper options (key=value)"]
#[arg (long = "option" ,
     value_name = "KEY=VALUE")] options : Vec < String > ,
...
#[arg (short ,
     long)] schema : Option < PathBuf > ,

#[doc = " History file"]
#[arg (long)] history : Option < PathBuf > ,

#[doc = " Startup script"]
#[arg (long)] init : Option < PathBuf > ,
...
#[doc = " Union of all elements"] Union ,

#[doc = " Intersection of common elements"] Intersection ,
...
#[doc = " Error on conflicts"] Error ,

#[doc = " Use first definition"] First ,
...
#[doc = " Unified diff format"] Unified ,

#[doc = " Side-by-side diff"] SideBySide ,

#[doc = " JSON patch format"] JsonPatch ,
...
#[doc = " Pretty printed output"] Pretty ,
...
#[doc = " No authentication"] None ,

#[doc = " Basic authentication"] Basic ,
...
#[doc = " CSV/TSV format"] Csv ,

#[doc = " JSON format"] Json ,

#[doc = " YAML format"] Yaml ,
...
if self . cli . verbose  {
tracing_subscriber ::  fmt () . with_env_filter ("linkml=debug") . init () ;

}
else  {
tracing_subscriber ::  fmt () . with_env_filter ("linkml=info") . init () ;
...
let collection =
    if data . is_array ()  {
data . as_array () . expect ("Operation failed") . clone ()
}
else  {
vec ! [data . clone ()]
}
;
validator . validate_collection_parallel (& collection ,
     target_class ,
     None) . await ?
}
else  {
validator . validate_as_class (& data ,
     target_class ,
     None) . await ?
}
;
...
for (i ,
     error) in report . issues . iter () . take (max_errors) . enumerate ()  {
println ! ("     {
...
" ,
     all_errors . len ()) ;
let mut error_types : HashMap < String ,
     usize > = HashMap ::  new () ;
...
#[doc = " Generate command implementation"] async
fn generate_command (& self ,
     schema_path : & Path ,
     output : & Path ,
     generator_name : & str ,
     options : & [String] ,
...
if
    let Some ((key ,
     value)) = opt . split_once ('=')  {
gen_options = gen_options . set_custom (key ,
     value) ;
...
if
    let Some (template_path) = template_dir  {
gen_options = gen_options . set_custom ("template_dir" ,
     template_path . to_str () . unwrap_or ("")) ;

}
if include_imports  {
let _ = gen_options . set_custom ("include_imports" ,
     "true") ;
...
#[doc = " Convert command implementation"] async
fn convert_command (& self ,
     input : & Path ,
     output : & Path ,
...
std ::  fs ::  write (output ,
     output_content) ? ;
println ! ("✓ Converted schema written to:  {
...
#[doc = " Merge command implementation"] async
fn merge_command (& self ,
     schemas : & [PathBuf] ,
     output : & Path ,
...
let base_schema = if
    let Some (base_path) = base  {
println ! ("Loading base schema:  {
...
std ::  fs ::  write (output ,
     output_content) ? ;
println ! ("✓ Merged schema written to:  {
...
let diff_result = differ . diff (& schema1 ,
     & schema2) ? ;
let formatted_diff =
    match format  {
...
#[doc = " Lint command implementation"] async
fn lint_command (& self ,
     schema_path : & Path ,
     rules : & [String] ,
     config : Option < & Path > ,
     fix : bool ,
...
if
    let Some (config_path) = config  {
println ! ("Loading lint config:  {

}
" ,
     config_path . display ()) ;
let config_content = std ::  fs ::  read_to_string (config_path) ? ;
let lint_config : HashMap < String ,
     serde_json ::  Value > = serde_yaml ::  from_str (& config_content) ? ;
...
std ::  fs ::  write (schema_path ,
     fixed_content) ? ;
println ! ("✓ Updated schema written to:  {
...
return Err (linkml_core ::  error ::  LinkMLError ::  service ("Both cert and key must be provided for TLS" ,
    )) ;
...
if
    let Some ((key ,
     value)) = opt . split_once ('=')  {
load_options . insert (key . to_string () ,
     value . to_string ()) ;
...
if
    let Some (has_header) = load_options . get ("header")  {
...
loader . load_file (input ,
     & schema ,
     & load_opts) . await . map_err (| e | LinkMLError ::  other (e . to_string ())) ?
}
LoadFormat ::  Json =>  {
...
if
    let Some (format_str) = load_options . get ("format")  {
rdf_options . format =
...
db_options . connection_string = load_options . get ("connection") . ok_or_else (| |  {
...
LoadFormat ::  TypeDb =>  {
return Err (LinkMLError ::  config ("TypeDB loading is not supported in the CLI. \n\
                     TypeDB operations require the DBMS service which is not available in standalone CLI mode. \n\
                     Please
use the LinkML service API with proper service dependencies instead." . to_string ())) ;
...
let report = validator . validate_as_class (& data ,
     target_class ,
     None) . await ? ;
if report . valid  {
valid_count += 1 ;
...
#[doc = " Dump command implementation"] async
...
if
    let Some ((key ,
     value)) = opt . split_once ('=')  {
dump_options . insert (key . to_string () ,
     value . to_string ()) ;
...
dumper . dump_file (& instances ,
     output ,
     & schema ,
     & dump_opts) . await . map_err (| e | LinkMLError ::  other (e . to_string ())) ? ;
...
let mut rdf_options = RdfOptions ::  default () ;
if
    let Some (format_str) = dump_options . get ("format")  {
rdf_options . format =
    match format_str . as_str ()  {
"turtle" => RdfSerializationFormat ::  Turtle ,
     "ntriples" => RdfSerializationFormat ::  NTriples ,
...
#[cfg (feature = "database")]  {
let mut db_options = DatabaseOptions ::  default () ;
db_options . connection_string = dump_options . get ("connection") . ok_or_else (| |  {
LinkMLError ::  config ("Database connection string required" . to_string ())
}
) ? . clone () ;
...
DumpFormat ::  TypeDb =>  {
return Err (LinkMLError ::  config ("TypeDB dumping is not supported in the CLI. \n\
                     TypeDB operations require the DBMS service which is not available in standalone CLI mode. \n\
                     Please
use the LinkML service API with proper service dependencies instead." . to_string ())) ;
...
#[doc = " Shell command implementation"] async
fn shell_command (& self ,
...
" ,
     "LinkML Interactive Shell" . bold () . blue ()) ;
println ! (" {
...
" ,
     "Syntax highlighting: enabled" . green ()) ;

}
let mut current_schema = if
    let Some (schema_path) = initial_schema  {
println ! ("Loading initial schema:  {
...
let mut rl = rustyline :: Editor ::  < () ,
     rustyline ::  history ::  DefaultHistory > ::  new () . map_err (| e | LinkMLError ::  other (e . to_string ())) ? ;
if
    let Some (history_path) = history_file  {
let _ = rl . load_history (history_path) ;

}
if
    let Some (init_path) = init_script  {
...
self . execute_shell_command (line ,
     & mut current_schema) . await ? ;
...
if
    let Err (e) = self . execute_shell_command (& line ,
     & mut current_schema) . await  {
eprintln ! (" {
...
#[doc = " Print lint results in pretty format"]
fn print_lint_results (& self ,
     result : & LintResult)  {
...
" ,
     ...
     issue . message) ;
if
    let Some (suggestion) = & issue . suggestion  {
println ! ("     {
...
#[doc = " Execute a shell command"] async
