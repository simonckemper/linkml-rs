...
impl RdfLoader {
    ...
    fn parse_rdf(&self, data: &[u8]) -> LoaderResult<Store> {
        let store = Store::new().map_err(|e| LoaderError::Io(
            std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to create store: {}", e))
        ))?;
        ...
        let parser = if let Some(base) = &self.options.base_iri {
            parser.with_base_iri(base).map_err(|e| LoaderError::Configuration(
    ...
    fn extract_instances(
        ...
        if options.infer_types {
            for quad_result in store.iter() {
                ...
                if let Some(class_name) = self.infer_class_from_properties(&quad.subject, store, schema) {
                    let mut data = HashMap::new();
    ...
    fn infer_class_from_types(&self, types: &[String], schema: &SchemaDefinition) -> LoaderResult<String> {
        ...
        Err(LoaderError::SchemaValidation(
            format!("Could not find LinkML class for RDF types: {:?}", types)
        ))
    }
...
impl RdfDumper {
    ...
    fn create_store(
        ...
        let store = Store::new().map_err(|e| DumperError::Io(
            std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to create store: {}", e))
        ))?;
        ...
        for instance in instances {
            ...
            let class_node = NamedNode::new(&class_uri).map_err(|e| 
                DumperError::Serialization(format!("Invalid class URI: {}", e))
            )?;
            ...
            store.insert(&type_quad).map_err(|e| DumperError::Io(
                std::io::Error::new(std::io::ErrorKind::Other, format!("Failed to insert type quad: {}", e))
            ))?;
                ...
                match value {
                    JsonValue::Array(arr) => {
                        for item in arr {
                            ...
                            store.insert(&quad).map_err(|e| DumperError::Io(
    ...
    fn json_to_term(
        ...
        match value {
            JsonValue::Null => Err(DumperError::TypeConversion("Cannot convert null to RDF".to_string())),
            ...
            JsonValue::Array(_) => {
                Err(DumperError::TypeConversion(
...
impl DataDumper for RdfDumper {
    ...
    async fn dump_string(
        ...
        String::from_utf8(data).map_err(|e| 
            DumperError::Serialization(format!("Invalid UTF-8: {}", e))
        )
    }
