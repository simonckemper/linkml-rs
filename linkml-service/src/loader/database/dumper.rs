...
impl DatabaseDumper {
    ...
    async fn execute_query_as_maps(
        ...
        match self.pool.as_ref() {
            ...
            None => Err(DumperError::Configuration(
    ...
    async fn execute_statement(&self, statement: &str) -> DumperResult<()> {
        match self.pool.as_ref() {
            ...
            Some(DatabasePool::MySQL(pool)) => {
                sqlx::query(statement).execute(pool).await.map_err(|e| {
                    DumperError::Io(std::io::Error::other(format!("MySQL statement failed: {e}")))
                })?;
                Ok(())
            }
            None => Err(DumperError::Configuration(
        ...
        for slot_name in &class_def.slots {
            if let Some(slot_def) = schema.slots.get(slot_name) {
                ...
                let db_type = self.linkml_range_to_db_type(
                    &slot_def.range.clone().unwrap_or_else(|| "string".to_string()),
                )?;
