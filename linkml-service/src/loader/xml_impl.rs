...
use crate::loader::{DataLoader, LoadOptions, LoaderError, LoaderResult, DataInstance};
    ...
    fn parse_xml(&self, xml_content: &str) -> LoaderResult<Value> {
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Text(e)) => {
                    let text = e.unescape()
                        .map_err(|err| LoaderError::Parse(format!("XML text unescape error: {}", err)))?
                        .to_string();
            ...
            match child {
                Value::Object(child_obj) => {
                    if let Some(Value::String(child_type)) = child_obj.get("_type") {
                        ...
                        let entry = obj.entry(child_name.clone()).or_insert_with(|| Value::Array(Vec::new()));
                ...
                _ => {
                    // Simple value child
                    let children_entry = obj.entry("children".to_string())
...
impl DataLoader for XmlLoader {
    ...
    async fn load_string(
        ...
        let target_class = options.target_class.as_deref()
