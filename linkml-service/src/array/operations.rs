...
impl ArrayOperations for ArrayData {
    ...
    fn mean(&self) -> ArrayResult<f64> {
        if self.data.is_empty() {
            return Err(ArrayError::InvalidData("Cannot compute mean of empty array".to_string()));
        }
        let sum = self.sum()?;
    ...
    fn std_dev(&self) -> ArrayResult<f64> {
        if self.data.len() < 2 {
            return Err(ArrayError::InvalidData("Need at least 2 elements for standard deviation".to_string()));
        }
        ...
        if count < 2 {
            return Err(ArrayError::InvalidData("Need at least 2 numeric values".to_string()));
        }
...
impl ArrayData {
        ...
        F: Fn(&Value, &Value) -> Value,
    {
        ...
        let result_data: Vec<Value> = self.data.iter()
            .zip(other.data.iter())
            .map(|(a, b)| op(a, b))
            .collect();
    ...
    fn sort_1d(&self, descending: bool) -> ArrayResult<ArrayData> {
        ...
        let sorted_data: Vec<Value> = indexed_data.into_iter()
            .map(|(_, v)| v.clone())
            .collect();
...
mod tests {
    ...
    fn test_array_filter() {
        let spec = ArraySpec::new("integer")
            .with_dimension(ArrayDimension::fixed("x", 5));
    ...
    fn test_element_wise_operations() {
        ...
        let array2 = ArrayData::new(spec, vec![3], data2)
            .expect("test data should create valid array2");
        ...
        let product = array1.multiply(&array2)
    ...
    fn test_array_sorting() {
        ...
        let sorted = array.sort_along(0, false)
            .expect("sort should succeed on 1D array");
        assert_eq!(sorted.data, vec![json!(1), json!(1), json!(3), json!(4), json!(5)]);
        
        let sorted_desc = array.sort_along(0, true)
            .expect("descending sort should succeed on 1D array");
        assert_eq!(sorted_desc.data, vec![json!(5), json!(4), json!(3), json!(1), json!(1)]);
    }
}