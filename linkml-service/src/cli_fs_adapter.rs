...
impl<F: FileSystemOperations> CLIFileSystemOps<F> {
    ...
    /// # Errors
    ///
    pub async fn write_output(&self, path: &Path, contents: &str) -> linkml_core::error::Result<()> {
    ...
    /// # Errors
    ///
    pub async fn read_dir(&self, path: &Path) -> linkml_core::error::Result<Vec<std::path::PathBuf>> {
...
#[must_use] pub fn default_cli_fs() -> CLIFileSystemOps<crate::file_system_adapter::TokioFileSystemAdapter> {
