...
use linkml_core::types::{
    ClassDefinition, SchemaDefinition, SlotDefinition, 
    TypeDefinition, EnumDefinition, PermissibleValue
};
...
fn test_conflicting_class_definitions() {
    ...
    let merged = merger.merge(vec![schema1.clone(), schema2.clone()]).unwrap();
...
fn test_type_and_enum_merging() {
    ...
    // Add custom type
    let mut phone_type = TypeDefinition::new("PhoneNumber");
    ...
    // Add enum
    let mut status_enum = EnumDefinition::new("Status");
    ...
    let mut status_enum2 = EnumDefinition::new("Status");
    ...
    // Add another type
    let mut email_type = TypeDefinition::new("Email");
    ...
    assert!(status.permissible_values.iter().any(|v| v.text == "pending"));
}

#[test]
fn test_prefix_merging() {
    let mut schema1 = SchemaDefinition::new("schema1");
    schema1.prefixes.insert("ex".to_string(), "https://example.org/".to_string());
    schema1.prefixes.insert("schema".to_string(), "https://schema.org/".to_string());
    
    let mut schema2 = SchemaDefinition::new("schema2");
    schema2.prefixes.insert("ex".to_string(), "https://example.com/".to_string()); // Conflict
    schema2.prefixes.insert("bio".to_string(), "https://bio.org/".to_string());
    ...
    assert_eq!(merged.prefixes.get("ex"), Some(&"https://example.com/".to_string())); // Last wins
    assert_eq!(merged.prefixes.get("schema"), Some(&"https://schema.org/".to_string()));
    assert_eq!(merged.prefixes.get("bio"), Some(&"https://bio.org/".to_string()));
}

#[test]
fn test_subset_merging() {
    let mut schema1 = SchemaDefinition::new("schema1");
    schema1.subsets.insert("clinical".to_string(), vec!["Patient".to_string(), "Diagnosis".to_string()]);
    schema1.subsets.insert("research".to_string(), vec!["Study".to_string()]);
    
    let mut schema2 = SchemaDefinition::new("schema2");
    schema2.subsets.insert("clinical".to_string(), vec!["Treatment".to_string()]);
    schema2.subsets.insert("administrative".to_string(), vec!["Billing".to_string()]);
...
fn test_inheritance_preservation() {
    ...
    assert_eq!(merged.classes.get("Derived1").unwrap().is_a.as_deref(), Some("Base"));
    assert_eq!(merged.classes.get("Derived2").unwrap().is_a.as_deref(), Some("Base"));
}
...
fn test_mixin_merging() {
    ...
    schema1.classes.insert("Timestamped".to_string(), timestamped);
    ...
    assert!(merged.classes.get("Timestamped").unwrap().mixin.unwrap_or(false));
    assert!(merged.classes.get("Auditable").unwrap().mixin.unwrap_or(false));
...
fn test_circular_reference_handling() {
    ...
    schema2.classes.insert("Organization".to_string(), organization);
    ...
    assert_eq!(merged.slots.get("spouse").unwrap().range.as_deref(), Some("Person"));
    assert_eq!(merged.slots.get("parent").unwrap().range.as_deref(), Some("Organization"));
}
...
fn test_complex_multi_schema_merge() {
    ...
    clinical_schema.classes.insert("Patient".to_string(), patient);
    ...
    research_schema.classes.insert("StudyPatient".to_string(), study_patient);
    
    // Merge all three
    let merger = SchemaMerger::new();
    let merged = merger.merge(vec![base_schema, clinical_schema, research_schema]).unwrap();
    
    // Verify complex inheritance chain
    assert_eq!(merged.classes.len(), 3);
    assert!(merged.classes.get("Entity").unwrap().abstract_class.unwrap_or(false));
    assert_eq!(merged.classes.get("Patient").unwrap().is_a.as_deref(), Some("Entity"));
    assert_eq!(merged.classes.get("StudyPatient").unwrap().is_a.as_deref(), Some("Patient"));
}
