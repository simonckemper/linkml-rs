...
fn test_slot_metadata_inheritance() {
    ...
    schema
        .slots
        .insert("derived_slot".to_string(), derived_slot);
    ...
    assert!(
        metadata
            .see_also
            .contains(&"https://example.org/base".to_string())
    );
    assert!(
        metadata
            .see_also
            .contains(&"https://example.org/derived".to_string())
    );
...
fn test_schema_metadata_features() {
    ...
    assert_eq!(
        schema.schema_metadata.contributors[0].role.as_deref(),
        Some("lead")
    );
...
"#;
    ...
    assert_eq!(
        schema.schema_metadata.contributors[0].orcid.as_deref(),
    ...
    assert_eq!(
        patient_class.metadata.deprecated.as_deref(),
        Some("Use Person with patient_role instead")
    );
...
fn test_metadata_serialization_roundtrip() {
    ...
    // Serialize to YAML
    let yaml = serde_yaml::to_string(&metadata).expect("Test operation failed");

    // Deserialize back
    let deserialized: ElementMetadata = serde_yaml::from_str(&yaml).expect("Test operation failed");
