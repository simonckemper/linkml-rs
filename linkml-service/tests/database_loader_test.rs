...
use linkml_service::loader::{
    DatabaseLoader, DatabaseDumper, DatabaseOptions, ForeignKeyRelation,
    DataLoader, DataDumper, traits::DataInstance
};
...
fn test_database_options_configuration() {
    ...
    // Add table mappings
    options.table_mapping.insert("people".to_string(), "Person".to_string());
    options.table_mapping.insert("addresses".to_string(), "Address".to_string());
    ...
    options.column_mapping.insert("people".to_string(), person_columns);
    
    // Add foreign keys
    options.foreign_keys.insert("addresses".to_string(), vec![
        ForeignKeyRelation {
...
fn test_table_filtering() {
    ...
    // Test exclude tables
    options.exclude_tables = ["audit_log", "temp_table"].iter()
        .map(|s| s.to_string())
        .collect();
    ...
    // Test include tables
    options.include_tables = Some(["users", "orders"].iter()
        .map(|s| s.to_string())
        .collect());
...
fn test_complex_mapping_scenario() {
    ...
    options.table_mapping.insert("tbl_usr".to_string(), "User".to_string());
    options.table_mapping.insert("tbl_prd".to_string(), "Product".to_string());
    options.table_mapping.insert("tbl_ord".to_string(), "Order".to_string());
    ...
    options.column_mapping.insert("tbl_usr".to_string(), user_columns);
    ...
    options.column_mapping.insert("tbl_prd".to_string(), product_columns);
    
    // Multiple foreign keys
    options.foreign_keys.insert("tbl_ord".to_string(), vec![
        ForeignKeyRelation {
...
fn test_type_mapping() {
    ...
    assert_eq!(schema.slots.get("id").unwrap().range, Some("string".to_string()));
    assert_eq!(schema.slots.get("age").unwrap().range, Some("integer".to_string()));
    assert_eq!(schema.slots.get("active").unwrap().range, Some("boolean".to_string()));
    assert_eq!(schema.slots.get("person").unwrap().range, Some("Person".to_string()));
}
