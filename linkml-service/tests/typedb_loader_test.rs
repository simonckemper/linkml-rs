...
use linkml_service::loader::{
    ...
    TypeDBQueryExecutor, DataLoader, DataDumper, traits::DataInstance
};
...
impl TypeDBQueryExecutor for MockTypeDBExecutor {
    async fn execute_query(&self, query: &str, _database: &str) -> Result<String, Box<dyn std::error::Error>> {
...
fn create_test_schema() -> SchemaDefinition {
    ...
    schema.slots.insert("full_name".to_string(), name_slot.clone());
...
fn test_options_with_mappings() {
    ...
    // Add type mappings
    options.type_mapping.insert("person".to_string(), "Person".to_string());
    options.type_mapping.insert("company".to_string(), "Organization".to_string());
    ...
    options.attribute_mapping.insert("person".to_string(), person_attrs);
    
    assert_eq!(options.type_mapping.len(), 2);
    assert_eq!(options.attribute_mapping.len(), 1);
    assert_eq!(
        options.attribute_mapping.get("person").unwrap().get("full-name"),
