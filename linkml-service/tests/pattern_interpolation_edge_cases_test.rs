...
use linkml_core::types::{
    ...
    StructuredPatternDefinition, PatternSyntax
};
...
async fn test_nested_interpolation_sources() {
    ...
    let report = engine.validate_as_class(&data, "Company", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&bad_data, "Company", None).await.unwrap();
    assert!(!report.valid);
}
...
async fn test_multiple_interpolation_variables() {
    ...
    file_class.slots = vec!["project".to_string(), "version".to_string(), "extension".to_string(), "filename".to_string()];
    schema.classes.insert("File".to_string(), file_class);
    
    // Basic slots
    for (name, value) in [("project", "string"), ("version", "string"), ("extension", "string")] {
    ...
    let report = engine.validate_as_class(&bad_data, "File", None).await.unwrap();
    assert!(!report.valid);
}
...
async fn test_glob_pattern_interpolation() {
    ...
    schema.classes.insert("Resource".to_string(), resource_class);
    ...
    let test_cases = vec![
        ...
        ("/data/documents/reports/Q4/file_report.json", "documents", true),
    ...
    for (path, category, should_pass) in test_cases {
        ...
        let report = engine.validate_as_class(&data, "Resource", None).await.unwrap();
        assert_eq!(report.valid, should_pass, "Path {} with category {} should {}", 
                   path, category, if should_pass { "pass" } else { "fail" });
    }
}
...
async fn test_context_interpolation() {
    ...
    assert!(id_slot.structured_pattern.as_ref().unwrap().interpolated.unwrap_or(false));
}
...
async fn test_escaped_interpolation_brackets() {
    ...
    schema.classes.insert("Template".to_string(), template_class);
    ...
    let report = engine.validate_as_class(&data, "Template", None).await.unwrap();
    assert!(report.valid);
}
...
async fn test_circular_interpolation_references() {
    ...
    let report = engine.validate_as_class(&data, "Circular", None).await.unwrap();
...
async fn test_missing_interpolation_variable() {
    ...
    let report = engine.validate_as_class(&data, "Document", None).await.unwrap();
...
async fn test_interpolation_with_special_regex_chars() {
    ...
    for (key, pattern_value, _should_pass) in test_cases {
        ...
        let report = engine.validate_as_class(&data, "Config", None).await.unwrap();
...
async fn test_interpolation_performance_with_many_variables() {
    ...
    for _ in 0..100 {
        let report = engine.validate_as_class(&data, "BigClass", None).await.unwrap();
        assert!(report.valid);
    }
