...
async fn test_concurrent_unique_validation() {
    ...
    for i in 0..10 {
        ...
        let handle = task::spawn(async move {
            ...
            for j in 0..5 {
                ...
                let report = engine_clone.validate_as_class(&user, "User", None).await.unwrap();
    ...
    engine2.validate_as_class(&user1, "User", None).await.unwrap();
...
async fn test_composite_unique_keys_edge_cases() {
    ...
    order_class.slots = vec!["customer_id".to_string(), "order_date".to_string(), "product_id".to_string()];
    
    // Composite unique key
    order_class.unique_keys = vec![
        UniqueKeyDefinition {
            unique_key_name: "customer_date_product".to_string(),
            unique_key_slots: vec!["customer_id".to_string(), "order_date".to_string(), "product_id".to_string()],
    ...
    let report1 = engine.validate_as_class(&order1, "Order", None).await.unwrap();
    ...
    let report1_dup = engine.validate_as_class(&order1_dup, "Order", None).await.unwrap();
    ...
    let report2 = engine.validate_as_class(&order2, "Order", None).await.unwrap();
    ...
    let report3 = engine.validate_as_class(&order3, "Order", None).await.unwrap();
    ...
    let report4 = engine.validate_as_class(&order4, "Order", None).await.unwrap();
...
async fn test_unique_validation_with_updates() {
    ...
    let report1 = engine.validate_as_class(&product1, "Product", None).await.unwrap();
    assert!(report1.valid);
    ...
    let report_update = engine.validate_as_class(&product1_updated, "Product", None).await.unwrap();
    ...
    let report2 = engine.validate_as_class(&product2, "Product", None).await.unwrap();
    assert!(report2.valid);
}
...
async fn test_unique_keys_with_inheritance() {
    ...
    let report1 = engine.validate_as_class(&user1, "User", None).await.unwrap();
    assert!(report1.valid);
    ...
    let report2 = engine.validate_as_class(&user2, "User", None).await.unwrap();
    assert!(!report2.valid);
    ...
    let report3 = engine.validate_as_class(&user3, "User", None).await.unwrap();
    assert!(!report3.valid);
}
...
async fn test_unique_validation_memory_efficiency() {
    ...
    for i in 0..1000 {
        ...
        let report = engine.validate_as_class(&record, "Record", None).await.unwrap();
    ...
    let avg_time = validation_times.iter().sum::<std::time::Duration>() / validation_times.len() as u32;
...
async fn test_unique_keys_with_special_characters() {
    ...
    for path in test_paths {
        ...
        let report = engine.validate_as_class(&doc, "Document", None).await.unwrap();
        ...
        let report_dup = engine.validate_as_class(&doc_dup, "Document", None).await.unwrap();
        assert!(!report_dup.valid, "Duplicate not detected for path: {}", path);
    }
}
...
async fn test_unique_validation_with_empty_values() {
    ...
    let report1 = engine.validate_as_class(&config1, "Config", None).await.unwrap();
    assert!(report1.valid);
    ...
    let report2 = engine.validate_as_class(&config2, "Config", None).await.unwrap();
    assert!(!report2.valid);
    ...
    let report3 = engine.validate_as_class(&config3, "Config", None).await.unwrap();
    assert!(report3.valid);
    ...
    let report4 = engine.validate_as_class(&config4, "Config", None).await.unwrap();
