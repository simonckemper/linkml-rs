...
";
    ...
    assert!(validation_result.valid, "Valid instance should pass validation");
    ...
    assert!(!invalid_result.valid, "Invalid instance should fail validation");
    assert!(!invalid_result.issues.is_empty(), "Should have validation issues");
...
";
    ...
    assert!(validation.valid, "Person should inherit id slot from Entity");
