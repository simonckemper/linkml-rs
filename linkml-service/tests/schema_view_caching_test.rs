...
fn create_complex_schema() -> SchemaDefinition {
    ...
    address.slots = vec!["street".to_string(), "city".to_string(), "state".to_string(), "zip".to_string()];
    ...
    // Add all slots
    for slot_name in ["id", "name", "employee_id", "department", "street", "city", "state", "zip", "dept_id"] {
        let mut slot = SlotDefinition::new(slot_name);
        slot.range = Some("string".to_string());
        schema.slots.insert(slot_name.to_string(), slot);
    }
