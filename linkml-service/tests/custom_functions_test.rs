...
fn test_custom_function_registration() {
    ...
    let result = registry.call("uppercase", vec![json!("hello world")]).unwrap();
...
fn test_custom_function_in_expression() {
    ...
    evaluator.function_registry_mut().register_custom(CustomFunction::new(
        "reverse",
        1,
        Some(1),
        |args| {
    ...
    let expr = parser.parse("reverse(name)").unwrap();
...
fn test_custom_math_function() {
    ...
    // Test with integer
    let parser = Parser::new();
    let expr = parser.parse("square(5)").unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
    assert_eq!(result, json!(25));
    
    // Test with float
    let expr = parser.parse("square(3.5)").unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
...
fn test_custom_function_with_multiple_args() {
    ...
    evaluator.function_registry_mut().register_custom(CustomFunction::new(
        ...
        |args| {
            let strings: Result<Vec<String>, _> = args.iter()
                .map(|v| match v {
    ...
    let parser = Parser::new();
    let expr = parser.parse(r#"join("Hello", "World")"#).unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
    assert_eq!(result, json!("Hello World"));
    
    // Test with 3 arguments
    let expr = parser.parse(r#"join("One", "Two", "Three")"#).unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
...
fn test_custom_validation_function() {
    ...
    // Test valid email
    let parser = Parser::new();
    let expr = parser.parse(r#"is_email("user@example.com")"#).unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
    assert_eq!(result, json!(true));
    
    // Test invalid email
    let expr = parser.parse(r#"is_email("not-an-email")"#).unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
...
fn test_custom_function_error_handling() {
    ...
    let result = registry.call("add_days", vec![json!("2025-01-31"), json!(7)]).unwrap();
    assert_eq!(result, json!("2025-01-31 + 7 days"));
    
    // Test with wrong types
    let err = registry.call("add_days", vec![json!(123), json!("not a number")]).unwrap_err();
    assert!(err.message.contains("Expected (string, number) arguments"));
    
    // Test with wrong arity
    let err = registry.call("add_days", vec![json!("2025-01-31")]).unwrap_err();
...
fn test_custom_function_list_operations() {
                ...
                Value::Array(arr) => {
                    ...
                    for val in arr {
                        match val {
                            ...
                            _ => return Err(FunctionError::new("All array elements must be numbers")),
                        }
                    }
                    Ok(json!(sum))
                }
    ...
    // Test sum function
    let parser = Parser::new();
    let expr = parser.parse("sum(numbers)").unwrap();
...
fn test_custom_function_chaining() {
    ...
    let parser = Parser::new();
    let expr = parser.parse("add_one(double(5))").unwrap();
    let result = evaluator.evaluate(&expr, &std::collections::HashMap::new()).unwrap();
