...
use linkml_service::config::{
    get_config, load_config, load_default_config, load_production_config,
    load_environment_config, LinkMLConfig,
    validation::{validate_config, validate_values},
    hot_reload::{ConfigHotReloader, init_hot_reload, get_hot_config},
};
use std::env;
use std::fs;
use std::path::Path;
...
"#;
    ...
    let config: TestConfig = load_config(temp_file.path()).expect("should load test config with defaults");
...
fn test_config_validation_ttl_ordering() {
    ...
    assert!(result.unwrap_err().to_string().contains("L1 cache TTL must be less than L2"));
    ...
    let result = validate_values(&config);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("L2 cache TTL must be less than L3"));
}
...
fn test_config_validation_memory_limits() {
    ...
    let result = validate_values(&config);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Memory pool size cannot exceed total memory limit"));
    ...
    let result = validate_values(&config);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Cache memory limit cannot exceed total memory limit"));
}
...
"#;
    ...
    let mut reloader = ConfigHotReloader::new(&config_path)
        .expect("should create hot reloader");
    ...
    // Start watching
    reloader.start_watching().await.expect("should start watching");
...
fn test_production_config_higher_limits() {
    ...
    assert!(prod_config.performance.memory_limit_bytes > default_config.performance.memory_limit_bytes);
    assert!(prod_config.security_limits.max_cache_entries > default_config.security_limits.max_cache_entries);
    
    // Production should have longer TTLs
    assert!(prod_config.cache.ttl_seconds >= default_config.cache.ttl_seconds);
    assert!(prod_config.cache.expression_cache.ttl_seconds >= default_config.cache.expression_cache.ttl_seconds);
}