...
use linkml_core::{
    settings::{
        SchemaSettings, ValidationSettings, GenerationSettings, ImportSettings,
        DefaultSettings, NamingSettings, ImportResolutionStrategy
    },
...
async fn test_settings_override_precedence() {
    ...
    let report1 = engine.validate_as_class(&data, "Person", None).await.unwrap();
    ...
    let report2 = engine.validate_as_class(&data, "Person", Some(options)).await.unwrap();
...
async fn test_nested_settings_inheritance() {
    ...
    assert!(derived_schema.settings.as_ref().unwrap().validation.allow_additional_properties);
}
...
async fn test_generation_settings_with_naming() {
    ...
    assert_eq!(settings.naming.class_naming_convention.as_deref(), Some("PascalCase"));
}
...
async fn test_settings_serialization_roundtrip() {
    ...
    assert_eq!(settings.validation.fail_on_warning, deserialized.validation.fail_on_warning);
    assert_eq!(settings.generation.target_languages, deserialized.generation.target_languages);
    assert_eq!(settings.imports.import_aliases.get("bio"), deserialized.imports.import_aliases.get("bio"));
    assert_eq!(settings.naming.class_naming_convention, deserialized.naming.class_naming_convention);
...
async fn test_settings_with_inheritance_chain() {
    ...
    let report = engine.validate_as_class(&data, "Derived", None).await.unwrap();
...
async fn test_settings_performance_impact() {
    ...
    for _ in 0..100 {
        let _ = strict_engine.validate_as_class(&data, "TestClass", None).await.unwrap();
    }
    let strict_time = start.elapsed();
    
    let start = Instant::now();
    for _ in 0..100 {
        let _ = lenient_engine.validate_as_class(&data, "TestClass", None).await.unwrap();
    }
