...
use linkml_core::types::{SchemaDefinition, ClassDefinition, SlotDefinition, Rule, RuleConditions, SlotCondition};
...
fn create_test_schema() -> SchemaDefinition {
    ...
    slots.insert("age".to_string(), SlotDefinition {
        name: "age".to_string(),
    ...
    slots.insert("guardian_name".to_string(), SlotDefinition {
    ...
    slots.insert("guardian_phone".to_string(), SlotDefinition {
    ...
    slots.insert("status".to_string(), SlotDefinition {
    ...
    slots.insert("reason".to_string(), SlotDefinition {
    ...
    let mut person_class = ClassDefinition {
        ...
        slots: vec!["age".to_string(), "guardian_name".to_string(), "guardian_phone".to_string()],
        ..Default::default()
    };
...
async fn test_minor_without_guardian_fails() {
    ...
    let report = engine.validate_as_class(&instance, "Person", None).await.unwrap();
    ...
    assert!(messages.iter().any(|m| m.contains("guardian_name") && m.contains("required")));
    assert!(messages.iter().any(|m| m.contains("guardian_phone") && m.contains("required")));
}
...
async fn test_adult_with_guardian_fails() {
    ...
    let messages: Vec<_> = report.issues.iter().map(|i| &i.message).collect();
    assert!(messages.iter().any(|m| m.contains("Adults should not have guardian")));
}
...
async fn test_inactive_account_without_reason_fails() {
    ...
    let report = engine.validate_as_class(&instance, "Account", None).await.unwrap();
    
    assert!(!report.valid);
    assert_eq!(report.stats.error_count, 1);
    
    let messages: Vec<_> = report.issues.iter().map(|i| &i.message).collect();
    assert!(messages.iter().any(|m| m.contains("reason") && m.contains("required")));
}
...
async fn test_fail_fast_option() {
    ...
    let report = engine.validate_as_class(&instance, "Person", Some(options)).await.unwrap();
