...
use linkml_service::expression::{
    ...
    functions::{
        AggregationFunctions, DateFunctions, FunctionRegistry, MathFunctions, StringFunctions,
    },
...
fn test_expression_parser_error_handling() {
    ...
    let result = parser.parse_str("add(1, 2");
    assert!(result.is_err());

    // Test invalid operator
    let result = parser.parse_str("1 ??? 2");
    assert!(result.is_err());

    // Test unclosed string
    let result = parser.parse_str(r#"concat("hello, "world")"#);
    assert!(result.is_err());

    // Test invalid function name
    let result = parser.parse_str("123invalid_function()");
    assert!(result.is_err());
...
fn test_expression_validation() {
    ...
    for (expr, description) in test_cases {
        if let Ok(parsed) = parser.parse_str(expr) {
