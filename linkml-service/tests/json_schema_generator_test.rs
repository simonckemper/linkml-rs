...
use linkml_service::generator::{JsonSchemaGenerator, Generator, GeneratorOptions};
...
async fn test_basic_json_schema_generation() {
    ...
    assert_eq!(person_def["properties"]["name"]["description"], "Person's full name");
    ...
    assert_eq!(person_def["properties"]["email"]["pattern"], r"^[^@]+@[^@]+\.[^@]+$");
    
    // Check required fields
    assert!(person_def["required"].as_array().unwrap().contains(&json!("name")));
}
...
async fn test_enum_json_schema() {
    ...
    assert_eq!(order_def["properties"]["status"]["$ref"], "#/definitions/OrderStatus");
}
...
async fn test_custom_types_json_schema() {
    ...
    assert_eq!(product_def["properties"]["website"]["$ref"], "#/definitions/URL");
    assert_eq!(product_def["properties"]["price"]["$ref"], "#/definitions/Price");
}
...
async fn test_additional_properties_handling() {
    ...
    schema.classes.insert("StrictClass".to_string(), strict_class);
