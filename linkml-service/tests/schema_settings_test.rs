...
async fn test_strict_validation_settings() {
    ...
    let report = engine.validate_as_class(&data_with_extra, "Person", None).await.unwrap();
    assert!(!report.valid);
...
async fn test_options_override_settings() {
    ...
    let report = engine.validate_as_class(&invalid_data, "Status", None).await.unwrap();
    assert!(!report.valid);
    let errors: Vec<_> = report.errors().collect();
    ...
    let report = engine.validate_as_class(&invalid_data, "Status", Some(options)).await.unwrap();
    assert!(!report.valid);
    let errors: Vec<_> = report.errors().collect();
    println!("Errors without fail_fast: {:?}", errors.iter().map(|e| &e.message).collect::<Vec<_>>());
