...
async fn test_custom_validator_registration() {
    ...
    let report = engine.validate_as_class(&valid_data, "Person", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&invalid_data, "Person", None).await.unwrap();
    assert!(!report.valid);
...
async fn test_custom_format_validator() {
    ...
    let phone_validator = helpers::format_validator(
        "us_phone_validator",
        "US phone number",
        |s| {
    ...
    for phone in valid_phones {
        ...
        let report = engine.validate_as_class(&data, "Contact", None).await.unwrap();
...
async fn test_cross_field_validator() {
    ...
    let report = engine.validate_as_class(&valid_data, "Event", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&invalid_data, "Event", None).await.unwrap();
    assert!(!report.valid);
    let errors: Vec<_> = report.errors().collect();
...
async fn test_custom_validator_with_predicate() {
                        ...
                        if re.is_match(s) && s.len() < 10 {
                            issues.push(
                                linkml_service::validator::report::ValidationIssue::error(
                                    format!("Value '{}' matching pattern must be at least 10 characters", s),
                                    context.path(),
    ...
    let report = engine.validate_as_class(&valid_data, "Product", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&invalid_data, "Product", None).await.unwrap();
    assert!(!report.valid);
    let errors: Vec<_> = report.errors().collect();
