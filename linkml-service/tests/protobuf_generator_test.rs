...
use linkml_service::generator::{ProtobufGenerator, Generator, GeneratorOptions};
...
async fn test_basic_protobuf_generation() {
    ...
    // Check filename
    assert_eq!(proto.filename, "person_schema.proto");
    
    // Check content
    assert!(proto.content.contains("syntax = \"proto3\""));
    assert!(proto.content.contains("package person_schema"));
    assert!(proto.content.contains("// A human being"));
    assert!(proto.content.contains("message Person {"));
    assert!(proto.content.contains("// Person's full name"));
    assert!(proto.content.contains("string name = 1"));
    assert!(proto.content.contains("int64 age = 2"));
    assert!(proto.content.contains("string email = 3"));
    assert!(proto.content.contains("}"));
}
...
async fn test_enums_and_repeated_fields() {
    ...
    schema.enums.insert("Department".to_string(), department_enum);
    ...
    schema.slots.insert("project_ids".to_string(), projects_slot);
    ...
    assert!(proto.content.contains("enum Department {"));
    assert!(proto.content.contains("DEPARTMENT_UNSPECIFIED = 0"));
    assert!(proto.content.contains("ENGINEERING = 1"));
    assert!(proto.content.contains("MARKETING = 2"));
    assert!(proto.content.contains("SALES = 3"));
    assert!(proto.content.contains("HR = 4"));
    
    // Check message with repeated fields
    assert!(proto.content.contains("message Employee {"));
    assert!(proto.content.contains("string employee_id = 1"));
    assert!(proto.content.contains("Department department = 2"));
    assert!(proto.content.contains("repeated string skills = 3"));
    assert!(proto.content.contains("repeated int64 project_ids = 4"));
}
...
async fn test_inheritance_and_composition() {
    ...
    assert!(proto.content.contains("import \"google/protobuf/timestamp.proto\""));
    
    // Check Person has inherited fields
    assert!(proto.content.contains("message Person {"));
    assert!(proto.content.contains("string id = 1"));
    assert!(proto.content.contains("string created_at = 2")); // datetime becomes string
    assert!(proto.content.contains("string name = 3"));
    assert!(proto.content.contains("string email = 4"));
    
    // Check Organization
    assert!(proto.content.contains("message Organization {"));
    assert!(proto.content.contains("repeated Person members = 4"));
}
...
async fn test_complex_types() {
    ...
    // Check type mappings
    assert!(proto.content.contains("string website = 1")); // Custom types map to base type
    assert!(proto.content.contains("int64 count = 2"));
    assert!(proto.content.contains("bool is_active = 3"));
    assert!(proto.content.contains("double price = 4"));
    assert!(proto.content.contains("// JSON metadata"));
    assert!(proto.content.contains("string metadata = 5"));
}
...
async fn test_case_handling() {
    ...
    schema.slots.insert("requestMethod".to_string(), method_slot);
    ...
    assert!(proto.content.contains("package case_test_schema"));
    
    // Check enum name stays PascalCase
    assert!(proto.content.contains("enum HTTPMethod"));
    assert!(proto.content.contains("HTTP_METHOD_UNSPECIFIED = 0"));
    assert!(proto.content.contains("GET = 1"));
    assert!(proto.content.contains("POST = 2"));
    assert!(proto.content.contains("PUT = 3"));
    assert!(proto.content.contains("DELETE = 4"));
    assert!(proto.content.contains("PATCH_REQUEST = 5"));
    
    // Check message name stays PascalCase
    assert!(proto.content.contains("message APIRequest"));
    
    // Check field names become snake_case
    assert!(proto.content.contains("HTTPMethod request_method = 1"));
    assert!(proto.content.contains("int64 response_code = 2"));
}