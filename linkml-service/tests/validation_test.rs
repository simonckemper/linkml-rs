...
"#;
    ...
    let report = validate_as_class(&schema, &valid_data, "Person", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = validate_as_class(&schema, &invalid_data, "Person", None).await.unwrap();
    assert!(!report.valid);
    assert!(report.stats.error_count > 0);
    
    // Find the required field error
    let required_error = report.errors()
...
"#;
    ...
    let report = validate_as_class(&schema, &wrong_types, "DataTypes", None).await.unwrap();
    assert!(!report.valid);
...
"#;
    ...
    let report = validate_as_class(&schema, &valid_data, "Contact", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = validate_as_class(&schema, &invalid_data, "Contact", None).await.unwrap();
    assert!(!report.valid);
...
"#;
    ...
    let report = validate_as_class(&schema, &invalid_data, "Measurement", None).await.unwrap();
    assert!(!report.valid);
    
    let temp_error = report.errors()
    ...
    let percent_error = report.errors()
...
"#;
    ...
    let report = validate_as_class(&schema, &valid_data, "Item", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = validate_as_class(&schema, &invalid_data, "Item", None).await.unwrap();
    assert!(!report.valid);
    
    let enum_error = report.errors()
...
"#;
    ...
    let report = engine.validate_as_class(&person_missing_inherited, "Person", None).await.unwrap();
    assert!(!report.valid);
