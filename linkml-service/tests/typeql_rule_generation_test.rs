...
fn create_test_schema() -> SchemaDefinition {
    ...
    age_category_slot.equals_expression = Some(r#"case({age} < 18, "minor", {age} < 65, "adult", "senior")"#.to_string());
    person.slot_usage.insert("age_category".to_string(), age_category_slot);
    
    // Add derived boolean
    let mut is_adult_slot = SlotDefinition::default();
    is_adult_slot.equals_expression = Some("{age} >= 18".to_string());
    person.slot_usage.insert("is_adult".to_string(), is_adult_slot);
    ...
    schema.slots.insert("employment_status".to_string(), employment_status_def);
    ...
    schema.slots.insert("age_category".to_string(), age_category_def);
...
async fn test_multiple_classes_with_rules() {
    ...
    employee.slot_usage.insert("employee_id".to_string(), employee_id_slot);
    ...
    schema.slots.insert("employee_id".to_string(), employee_id_def);
    
    let mut department_def = SlotDefinition::default();
    department_def.range = Some("string".to_string());
    schema.slots.insert("department".to_string(), department_def);
    ...
    assert!(!typeql.contains("## Validation Rules for Department") || 
            typeql.contains("dept_code @key"));
}
...
async fn test_expression_based_rules() {
    ...
    product.slot_usage.insert("total_price".to_string(), total_price_slot);
    ...
    product.slot_usage.insert("is_luxury".to_string(), is_luxury_slot);
    ...
    schema.slots.insert("total_price".to_string(), total_price_def);
    ...
    assert!(!typeql.contains("rule product-compute-total-price:") || 
