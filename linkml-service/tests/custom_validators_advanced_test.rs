...
use linkml_service::validator::{
    CustomValidator, CustomValidatorBuilder, AppliesTo, ValidationEngine, ValidationError
};
use serde_json::json;
...
async fn test_custom_validator_with_complex_applies_to() {
            ...
            if let Some(dept) = data.get("department").and_then(|v| v.as_str()) {
                ...
                if !valid_depts.contains(&dept) {
                    return vec![ValidationError::new(
                        format!("Invalid department: {}. Must be one of: {:?}", dept, valid_depts)
                    )];
                }
            }
            vec![]
    ...
    let report = engine.validate_as_class(&employee, "Employee", None).await.unwrap();
    assert!(!report.valid);
    assert!(report.errors().any(|e| e.message.contains("Invalid department")));
    ...
    let report = engine.validate_as_class(&person, "Person", None).await.unwrap();
...
async fn test_multiple_custom_validators_interaction() {
    ...
    user_class.slots = vec!["username".to_string(), "email".to_string(), "password".to_string()];
    schema.classes.insert("User".to_string(), user_class);
    
    for (name, range) in [("username", "string"), ("email", "string"), ("password", "string")] {
        let mut slot = SlotDefinition::new(name);
        slot.range = Some(range.to_string());
        schema.slots.insert(name.to_string(), slot);
    }
    ...
    let username_validator = CustomValidatorBuilder::new()
        ...
        .with_validation_fn(|data, _schema, _context| {
            ...
            if let Some(username) = data.get("username").and_then(|v| v.as_str()) {
                if username.len() < 3 {
                    errors.push(ValidationError::new("Username must be at least 3 characters"));
                }
                if username.contains(' ') {
                    errors.push(ValidationError::new("Username cannot contain spaces"));
                }
                if !username.chars().all(|c| c.is_alphanumeric() || c == '_') {
                    errors.push(ValidationError::new("Username can only contain letters, numbers, and underscores"));
                }
            }
            errors
        })
        ...
        .with_validation_fn(|data, _schema, _context| {
            ...
            if let Some(password) = data.get("password").and_then(|v| v.as_str()) {
                ...
                if !password.chars().any(|c| c.is_uppercase()) {
                    errors.push(ValidationError::new("Password must contain at least one uppercase letter"));
                }
                if !password.chars().any(|c| c.is_numeric()) {
                    errors.push(ValidationError::new("Password must contain at least one number"));
                }
            }
            errors
        })
...
async fn test_custom_validator_with_state() {
    ...
    transaction_class.slots = vec!["id".to_string(), "amount".to_string(), "user_id".to_string()];
    schema.classes.insert("Transaction".to_string(), transaction_class);
                ...
                *daily_total += amount;
                ...
                if *daily_total > 10000.0 {
                    errors.push(ValidationError::new(
                        format!("Daily transaction limit exceeded for user {}. Total: ${:.2}", user_id, daily_total)
                    ));
                }
            }
    ...
    let report = engine.validate_as_class(&tx1, "Transaction", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&tx2, "Transaction", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&tx3, "Transaction", None).await.unwrap();
    assert!(!report.valid);
    assert!(report.errors().any(|e| e.message.contains("Daily transaction limit exceeded")));
}
...
async fn test_custom_validator_with_async_validation() {
    ...
    schema.slots.insert("permissions".to_string(), permissions_slot);
    ...
    let report = engine.validate_as_class(&valid_key, "ApiKey", None).await.unwrap();
    assert!(report.valid);
    ...
    let report = engine.validate_as_class(&revoked_key, "ApiKey", None).await.unwrap();
    assert!(!report.valid);
...
async fn test_custom_validator_performance() {
    ...
    for _ in 0..100 {
        let report = engine.validate_as_class(&data, "Record", None).await.unwrap();
        assert!(report.valid);
    }
