...
#[test]
fn test_string_functions_integration() {
    ...
    // Test starts_with
    let result = engine.evaluate("starts_with(text, \"Hello\")", &context).unwrap();
    assert_eq!(result, json!(true));
    
    // Test ends_with
    let result = engine.evaluate("ends_with(text, \"World\")", &context).unwrap();
    assert_eq!(result, json!(true));
    
    // Test replace
    let result = engine.evaluate("replace(text, \"World\", \"Rust\")", &context).unwrap();
...
#[test]
fn test_date_functions_integration() {
    let engine = ExpressionEngine::new();
    let mut context = HashMap::new();
    ...
    // Test date_parse
    let result = engine.evaluate("date_parse(\"15/01/2024\", \"%d/%m/%Y\")", &context).unwrap();
    assert_eq!(result, json!("2024-01-15"));
    
    // Test date_format
    let result = engine.evaluate("date_format(date, \"%Y/%m/%d\")", &context).unwrap();
    assert_eq!(result, json!("2024/01/15"));
    
    // Test date_add
    let result = engine.evaluate("date_add(date, 10, \"days\")", &context).unwrap();
    assert_eq!(result, json!("2024-01-25"));
    
    let result = engine.evaluate("date_add(date, 2, \"months\")", &context).unwrap();
    assert_eq!(result, json!("2024-03-15"));
    
    // Test date_diff
    context.insert("date2".to_string(), json!("2024-01-25"));
    let result = engine.evaluate("date_diff(date, date2, \"days\")", &context).unwrap();
...
#[test]
fn test_math_functions_integration() {
    let engine = ExpressionEngine::new();
...
#[test]
fn test_aggregation_functions_integration() {
    let engine = ExpressionEngine::new();
    let mut context = HashMap::new();
    ...
    let result = engine.evaluate("count(mixed, \"non-null\")", &context).unwrap();
    assert_eq!(result, json!(3));
    ...
    // Test group_by
    context.insert("objects".to_string(), json!([
        {"type": "fruit", "name": "apple"},
        {"type": "vegetable", "name": "carrot"},
        {"type": "fruit", "name": "banana"}
    ]));
    let result = engine.evaluate("group_by(objects, \"type\")", &context).unwrap();
...
fn test_complex_expressions() {
    ...
    let result = engine.evaluate(
        "round(avg(scores) + stddev(scores), 2)",
        &context
    ).unwrap();
