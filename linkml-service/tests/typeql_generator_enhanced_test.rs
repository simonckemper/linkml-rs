...
async fn test_inheritance_generation() {
    ...
    schema.classes.insert("NamedEntity".to_string(), entity_class);
...
async fn test_mixin_generation() {
    ...
    schema.classes.insert("Timestamped".to_string(), timestamped);
...
async fn test_migration_generation() {
    ...
    assert!(migration.content.contains("# TypeQL Migration Script"));
    assert!(migration.content.contains("# Schema: TestSchema"));
}
