...
fn test_basic_rule_generation() {
    ...
    assert!(required_rule.to_typeql().contains("not { $x has name $v; }"));
    assert!(required_rule.to_typeql().contains("validation-error"));
    
    // Check for range rule
    let range_rule = rules.iter()
    ...
    assert!(range_rule.to_typeql().contains("$v < 0") || range_rule.to_typeql().contains("$v > 150"));
}
...
fn test_conditional_requirement_rule() {
    ...
    let conditional_rule = rules.iter()
    ...
    assert!(conditional_rule.to_typeql().contains("$x has employment_status $cond"));
    assert!(conditional_rule.to_typeql().contains("$cond = \"employed\""));
    assert!(conditional_rule.to_typeql().contains("not { $x has employer $v; }"));
}
...
fn test_rule_typeql_output() {
    ...
    let rule = TypeQLRule {
        ...
        then_patterns: vec![
            "$x has validation-error \"Missing required field: name\"".to_string(),
        ],
