...
use linkml_service::generator::{ShaclGenerator, Generator, GeneratorOptions};
...
async fn test_basic_shacl_generation() {
    ...
    // Check filename
    assert_eq!(shacl.filename, "person_schema.shacl");
    
    // Check content
    assert!(shacl.content.contains("# SHACL Shapes generated from LinkML schema"));
    assert!(shacl.content.contains("@prefix sh: <http://www.w3.org/ns/shacl#>"));
    assert!(shacl.content.contains("@prefix person_schema: <https://example.org/person#>"));
    
    // Check PersonShape
    assert!(shacl.content.contains("person_schema:PersonShape"));
    assert!(shacl.content.contains("a sh:NodeShape"));
    assert!(shacl.content.contains("sh:targetClass person_schema:Person"));
    ...
    assert!(shacl.content.contains("sh:pattern \"^[^@]+@[^@]+\\.[^@]+$\""));
}
...
async fn test_enum_shacl() {
    ...
    assert!(shacl.content.contains("sh:in (\"pending\" \"processing\" \"shipped\" \"delivered\")"));
}
...
async fn test_inheritance_shacl() {
    ...
    assert!(shacl.content.contains("entity_schema:PersonShape-created_at"));
...
async fn test_multivalued_shacl() {
    ...
    let members_section = shacl.content.split("team_schema:TeamShape-members").nth(1).unwrap_or("");
    let next_section_start = members_section.find("team_schema:").unwrap_or(members_section.len());
    let members_shape_content = &members_section[..next_section_start];
    assert!(!members_shape_content.contains("sh:maxCount"), "Multivalued field should not have maxCount");
}
...
async fn test_object_references_shacl() {
    ...
    org_class.slots = vec!["name".to_string(), "ceo".to_string(), "employees".to_string()];
    ...
    assert!(shacl.content.split("org_schema:OrganizationShape-employees").nth(1).unwrap().contains("sh:class org_schema:Person"));
}
