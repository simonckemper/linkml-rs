...
use linkml_service::performance::{intern, global_interner, global_memory_profiler};
use linkml_service::security::input_validation::{validate_string_input, limits};
...
fn test_function_registry_locking() {
    ...
    assert!(registry.register_custom(CustomFunction::new(
        "test1",
        1,
        Some(1),
...
fn test_restricted_function_registry() {
    ...
    assert!(registry.call("max", vec![json!(1), json!(2), json!(3)]).is_ok());
