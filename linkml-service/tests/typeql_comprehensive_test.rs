...
fn create_comprehensive_schema() -> SchemaDefinition {
    ...
    schema.slots.insert("participants".to_string(), participants_slot);
    ...
    schema.slots.insert("scheduled_time".to_string(), scheduled_time);
    ...
    schema.slots.insert("duration_minutes".to_string(), duration);
...
fn test_complex_constraints() {
    ...
    test_class.slot_usage.insert("complex_field".to_string(), usage);
    
    schema.slots.insert("complex_field".to_string(), complex_slot);
...
fn test_performance_scaling() {
    ...
    for size in sizes {
        ...
        if size == 100 {
            assert!(duration.as_millis() < 100, "Failed 100ms target for 100 classes");
        }
    }
}