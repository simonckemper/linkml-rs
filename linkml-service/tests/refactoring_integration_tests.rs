...
"#;
    ...
    fs_adapter.write_file(&schema_path, schema_content.as_bytes()).await?;
...
"#;
    ...
    fs_adapter.write_file(&valid_path, valid_schema.as_bytes()).await?;
    ...
    fs_adapter.write_file(&invalid_path, invalid_schema.as_bytes()).await?;
...
async fn test_expression_engine_no_panics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    schema.classes.insert("ExpressionTest".to_string(), class_def);
...
"#;
    ...
    let class_names: Vec<_> = schema_v2.classes.values()
...
"#)?;
    ...
    std::fs::write(&invalid_data_path, r#"
...
"#)?;
    ...
    // Generate code
    let python_code = service.generate_code(&schema, GeneratorType::Python).await?;
...
"#, i, i * 100))?;
    ...
    let parallel_results = service.validate_files_parallel(&data_files, &schema).await?;
...
async fn test_error_handling_no_panics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parser = SchemaParser::new(fs_adapter.clone());
    let result = parser.parse_file(Path::new("/non/existent/file.yaml")).await;
    assert!(result.is_err());
    match result {
    ...
    fs_adapter.write_file(&malformed_path, b"{ invalid yaml }}}").await?;
    ...
    fs_adapter.write_file(&schema1_path, r#"
...
"#.as_bytes()).await?;
    ...
    fs_adapter.write_file(&schema2_path, r#"
