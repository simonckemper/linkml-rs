...
fn create_test_schema() -> SchemaDefinition {
    ...
    schema.classes.insert("DerivedClass".to_string(), derived_class);
    ...
    schema.classes.insert("TimestampMixin".to_string(), mixin_class);
...
fn test_class_inheritance() {
    ...
    // Test ancestors
    let ancestors = view.class_ancestors("DerivedClass").expect("Failed to get ancestors");
    assert_eq!(ancestors, vec!["BaseClass".to_string()]);
    
    // Test descendants
    let descendants = view.class_descendants("BaseClass").expect("Failed to get descendants");
...
fn test_induced_class() {
    ...
    let induced = view.induced_class("DerivedClass").expect("Failed to get induced class");
...
fn test_class_with_mixins() {
    ...
    let induced = view.induced_class("MixedClass").expect("Failed to get induced class");
...
fn test_is_inlined() {
    ...
    let base_inlined = view.is_inlined("BaseClass").expect("Failed to check inlined");
    assert!(!base_inlined);
    
    // Mixins without identifiers should be inlined
    let mixin_inlined = view.is_inlined("TimestampMixin").expect("Failed to check inlined");
...
fn test_class_navigator() {
    ...
    // Test root classes
    let roots = navigator.get_root_classes().expect("Failed to get root classes");
    assert!(roots.contains(&"BaseClass".to_string()));
    assert!(roots.contains(&"TimestampMixin".to_string()));
    
    // Test leaf classes
    let leaves = navigator.get_leaf_classes().expect("Failed to get leaf classes");
    assert!(leaves.contains(&"DerivedClass".to_string()));
    assert!(leaves.contains(&"MixedClass".to_string()));
    
    // Test inheritance chain
    let chain = navigator.get_inheritance_chain("DerivedClass")
...
fn test_schema_analyzer() {
    ...
    // Test statistics
    let stats = analyzer.compute_statistics().expect("Failed to compute statistics");
    ...
    // Test pattern search
    let pattern_results = analyzer.find_elements_by_pattern(".*Class")
...
fn test_usage_index() {
    ...
    let base_usage = usage_index.get_usage("BaseClass").expect("No usage for BaseClass");
    assert_eq!(base_usage.used_by_classes.len(), 2);
    assert!(base_usage.used_by_classes.contains(&"DerivedClass".to_string()));
    assert!(base_usage.used_by_classes.contains(&"MixedClass".to_string()));
    
    // TimestampMixin should be marked as used as mixin
    let mixin_usage = usage_index.get_usage("TimestampMixin")
...
fn test_schema_view_caching() {
    ...
    let induced1 = view.induced_class("DerivedClass").expect("Failed to get induced class");
    
    // Second call should use cache (we can't directly test this, but it should be faster)
    let induced2 = view.induced_class("DerivedClass").expect("Failed to get induced class");
