...
use linkml_service::generator::{OwlRdfGenerator, Generator, GeneratorOptions};
...
async fn test_basic_owl_generation() {
    ...
    // Check content
    assert!(owl.content.contains("# OWL Ontology generated from LinkML schema"));
    assert!(owl.content.contains("@prefix owl: <http://www.w3.org/2002/07/owl#>"));
    ...
    assert!(owl.content.contains("xsd:pattern \"^[^@]+@[^@]+\\.[^@]+$\""));
...
async fn test_inheritance_owl() {
    ...
    assert!(owl.content.contains("owl:onProperty entity_schema:created_at"));
...
async fn test_multivalued_owl() {
    ...
    let next_property = members_section.find("# Property:").unwrap_or(members_section.len());
...
async fn test_property_domains_owl() {
    ...
    assert!(owl.content.contains("domain_schema:Person domain_schema:Organization domain_schema:Product"));
}