...
use linkml_core::types::{ClassDefinition, ConditionalRequirement, SchemaDefinition, SlotCondition, SlotDefinition};
...
async fn test_conditional_requirements_basic() {
    ...
    let person_class = ClassDefinition {
        ...
        slots: vec!["status".to_string(), "email".to_string(), "phone".to_string()],
    ...
    let report1 = engine.validate_as_class(&data1, "Person", Some(options.clone())).await
        .expect("Validation failed");
    
    assert!(!report1.valid, "Should be invalid when conditional requirements not met");
    let errors1: Vec<_> = report1.errors().collect();
    assert_eq!(errors1.len(), 2, "Should have 2 errors for missing email and phone");
    ...
    let report2 = engine.validate_as_class(&data2, "Person", Some(options.clone())).await
        .expect("Validation failed");
    
    assert!(report2.valid, "Should be valid when conditional requirements are met");
    ...
    let report3 = engine.validate_as_class(&data3, "Person", Some(options)).await
        .expect("Validation failed");
...
async fn test_conditional_requirements_numeric_range() {
    ...
    schema.slots.insert("guardian_name".to_string(), SlotDefinition {
        name: "guardian_name".to_string(),
        range: Some("string".to_string()),
        ..Default::default()
    });
    
    schema.slots.insert("driver_license".to_string(), SlotDefinition {
    ...
    let person_class = ClassDefinition {
        name: "Person".to_string(),
        slots: vec!["age".to_string(), "guardian_name".to_string(), "driver_license".to_string()],
        if_required: Some(if_required),
        ..Default::default()
    };
    ...
    assert!(report1.errors().any(|e| e.message.contains("guardian_name")));
...
async fn test_conditional_requirements_pattern() {
    ...
    schema.slots.insert("employee_id".to_string(), SlotDefinition {
        name: "employee_id".to_string(),
        range: Some("string".to_string()),
        ..Default::default()
    });
    
    schema.slots.insert("department".to_string(), SlotDefinition {
    ...
    let person_class = ClassDefinition {
        name: "Person".to_string(),
        slots: vec!["email".to_string(), "employee_id".to_string(), "department".to_string()],
        if_required: Some(if_required),
        ..Default::default()
    };
    ...
    assert!(report2.valid, "External email should not require employee info");
}
...
async fn test_conditional_requirements_field_presence() {
    ...
    // Add slots
    schema.slots.insert("shipping_address".to_string(), SlotDefinition {
        name: "shipping_address".to_string(),
        range: Some("string".to_string()),
        ..Default::default()
    });
    
    schema.slots.insert("shipping_method".to_string(), SlotDefinition {
        name: "shipping_method".to_string(),
        range: Some("string".to_string()),
        ..Default::default()
    });
    
    schema.slots.insert("tracking_email".to_string(), SlotDefinition {
    ...
    if_required.insert(
        ...
        ConditionalRequirement {
            ...
            then_required: Some(vec!["shipping_method".to_string(), "tracking_email".to_string()]),
        },
    );
    
    let order_class = ClassDefinition {
        name: "Order".to_string(),
        slots: vec!["shipping_address".to_string(), "shipping_method".to_string(), "tracking_email".to_string()],
        if_required: Some(if_required),
        ..Default::default()
    };
    ...
    let report1 = engine.validate_as_class(&data1, "Order", Some(options.clone())).await
        .expect("Validation failed");
    ...
    let report2 = engine.validate_as_class(&data2, "Order", Some(options.clone())).await
        .expect("Validation failed");
    
    assert!(report2.valid, "Digital order without shipping should be valid");
    ...
    let report3 = engine.validate_as_class(&data3, "Order", Some(options)).await
        .expect("Validation failed");
...
async fn test_multiple_conditional_requirements() {
    ...
    for slot_name in ["country", "state", "zip_code", "is_business", "tax_id", "business_name"] {
        schema.slots.insert(slot_name.to_string(), SlotDefinition {
    ...
    let address_class = ClassDefinition {
        ...
        slots: vec![
            "country".to_string(), "state".to_string(), "zip_code".to_string(),
            "is_business".to_string(), "tax_id".to_string(), "business_name".to_string()
        ],
        if_required: Some(if_required),
        ..Default::default()
    };
    ...
    let report = engine.validate_as_class(&data, "Address", Some(options)).await
        .expect("Validation failed");
    
    assert!(!report.valid, "Should have multiple conditional requirement violations");
