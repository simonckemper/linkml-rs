...
use mock_services::{
    MockLogger, MockTimestamp, MockCache, MockMonitor,
    MockTaskManager, MockErrorHandler, MockConfigService,
    MockDBMS, MockTimeout,
};
...
mod mock_services {
    ...
    impl logger_core::LoggerService for MockLogger {
        ...
        async fn trace(&self, _msg: &str) -> Result<(), Self::Error> { Ok(()) }
    ...
    impl cache_core::CacheService for MockCache {
        ...
        async fn set(&self, key: &str, value: Vec<u8>, _ttl: Option<u64>) -> Result<(), Self::Error> {
    ...
    impl task_management_core::TaskManagementService for MockTaskManager {
        ...
        async fn cancel_task(&self, _id: &task_management_core::TaskId) -> Result<bool, Self::Error> {
            Ok(true)
        }
        
        async fn get_task_status(&self, _id: &task_management_core::TaskId) -> Result<task_management_core::TaskStatus, Self::Error> {
...
pub async fn initialize_example_service() -> Arc<LinkMLServiceImpl<
    MockTaskManager,
    MockErrorHandler,
    MockConfigService,
    MockDBMS,
...
>> {
    ...
    let service = LinkMLServiceImpl::with_config(config, deps)
        .expect("Failed to create LinkML service");
