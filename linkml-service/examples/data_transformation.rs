...
use linkml_service::loader::{
    CsvLoader, CsvDumper, RdfLoader, RdfDumper, 
    RdfOptions, RdfSerializationFormat,
    LoadOptions, DumpOptions
};
...
"#;
    ...
    let researchers = csv_loader.load_string(people_csv, &schema, &load_options).await?;
    ...
    rdf_options.prefixes.insert("foaf".to_string(), "http://xmlns.com/foaf/0.1/".to_string());
    rdf_options.prefixes.insert("schema".to_string(), "http://schema.org/".to_string());
    rdf_options.prefixes.insert("ex".to_string(), "http://research.example.org/".to_string());
    
    let rdf_dumper = RdfDumper::with_options(rdf_options.clone());
    let turtle = rdf_dumper.dump_string(&researchers, &schema, &DumpOptions::default()).await?;
    ...
    let ntriples = ntriples_dumper.dump_string(&researchers, &schema, &DumpOptions::default()).await?;
    ...
    let rdfxml = rdfxml_dumper.dump_string(&researchers, &schema, &DumpOptions::default()).await?;
...
"#;
    ...
    let organizations = org_loader.load_string(orgs_csv, &schema, &org_options).await?;
    ...
    let combined_turtle = rdf_dumper.dump_string(&all_instances, &schema, &DumpOptions::default()).await?;
    std::fs::write("knowledge_graph.ttl", &combined_turtle)?;
    println!("Created combined knowledge graph with {} instances", all_instances.len());
    ...
    let reloaded = rdf_loader.load_string(&combined_turtle, &schema, &LoadOptions {
        infer_types: true,
        ..Default::default()
    }).await?;
    ...
    // Separate by class
    let researchers_only: Vec<_> = reloaded.iter()
        .filter(|i| i.class_name == "Researcher")
        .cloned()
        .collect();
    
    let orgs_only: Vec<_> = reloaded.iter()
        .filter(|i| i.class_name == "Organization")
        .cloned()
        .collect();
    
    let researchers_csv = csv_dumper.dump_string(&researchers_only, &schema, &DumpOptions::default()).await?;
    let orgs_csv = csv_dumper.dump_string(&orgs_only, &schema, &DumpOptions::default()).await?;
...
"#;
    ...
    extended_schema.classes.insert("Project".to_string(), project_class);
    
    // Load projects
    let projects = rdf_loader.load_string(project_turtle, &extended_schema, &LoadOptions {
        infer_types: true,
        ..Default::default()
    }).await?;
    ...
    let classes: std::collections::HashSet<_> = all_instances.iter()
        .chain(&projects)
        .map(|i| &i.class_name)
        .collect();
...
fn create_knowledge_graph_schema() -> SchemaDefinition {
    ...
    schema.slots.insert("collaborators".to_string(), collaborators_slot);
