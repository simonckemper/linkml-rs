...
use linkml_service::{
    ...
    performance::{global_profiler, intern, MemoryScope},
    security::{validate_string_input, ResourceLimits, create_monitor},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for slot in &person_slots {
        println!("  - {}: {} {}", 
            ...
            if slot.required == Some(true) { "(required)" } else { "" }
        );
    }
    ...
    println!("✓ Python dataclasses generated ({} lines)", python.lines().count());
    
    // Generate Rust
    let rust = profiler.time("generate_rust", || {
        RustGenerator::new().generate(&schema)
    })?;
    ...
    println!("✓ String interning: 'FULL_TIME' interned (same ref: {})", 
    ...
    // Memory scope tracking
    {
        ...
        println!("✓ Batch validation: {} employees in {:?}", employees.len(), elapsed);
