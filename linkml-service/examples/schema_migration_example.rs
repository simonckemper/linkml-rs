...
fn main () {
    ...
    mut ("Product") {
        ...
        product . slot_usage . insert ("sku" . to_string () ,
        sku_slot) ;
    ...
    v2_schema . slots . insert ("sku" . to_string () ,
    sku) ;
    let
    mut category = SlotDefinition ::default () ;
    category . range = Some ("string" . to_string ()) ;
    v2_schema . slots . insert ("category" . to_string () ,
    category) ;
    v2_schema . slots . re
    move ("description") ;
    let
    ...
    v2_schema . classes . insert ("Category" . to_string () ,
    category_class) ;
    ...
    if ! migration . data_migrations . is_empty () {
        println ! ("\n=== Data Migrations ===") ;
        for (i ,
        data_migration) in migration . data_migrations . iter () . enumerate () {
            println ! ("\nStep {
            }: {
            }" ,
            i + 1 ,
            data_migration . description) ;
            println ! ("Query:\n{
            }" ,
