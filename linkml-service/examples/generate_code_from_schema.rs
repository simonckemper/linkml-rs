...
use linkml_service::generator::{
    Generator, GeneratorOptions, GeneratorRegistry, IndentStyle,
    OutputFormat, PydanticGenerator, PythonDataclassGenerator, RustGenerator,
};
use std::fs;
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let pydantic_code = pydantic_generator.generate(&schema, &pydantic_options).await?;
    ...
    if let Some(classes) = &schema.classes {
        if let Some(iso_entity) = classes.get("ISO3166Entity") {
            ...
            let focused_rust = rust_generator.generate(&focused_schema, &rust_options).await?;
            let focused_rust_path = PathBuf::from("/tmp/iso3166_entity.rs");
            fs::write(&focused_rust_path, &focused_rust)?;
            println!("\n  ✓ Generated focused Rust code: {}", focused_rust_path.display());
            
            // Generate focused Python code
            let focused_python = python_generator.generate(&focused_schema, &python_options).await?;
            let focused_python_path = PathBuf::from("/tmp/iso3166_entity.py");
            fs::write(&focused_python_path, &focused_python)?;
            println!("  ✓ Generated focused Python code: {}", focused_python_path.display());
        }
    }
