...
use linkml_service::array::{ArraySpec, ArrayDimension, ArrayData, ArrayValidator};
use linkml_service::generator::array_support::{get_array_generator, ArrayCodeGenerator};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Dimensions: {:?}", imaging_spec.dimensions.iter()
        .map(|d| format!("{} ({})", d.name, d.size.unwrap()))
        .collect::<Vec<_>>());
    println!("  Element type: {}", imaging_spec.element_type);
    println!("  Total elements: {}", ArraySpec::calculate_size(&[50, 512, 512]));
    println!();
    ...
    schema.classes.insert("Measurement".to_string(), measurement_class);
    ...
    match ArrayData::new(
        test_spec.clone(),
        vec![2, 3], // Wrong shape!
        vec![json!(1.0), json!(2.0), json!(3.0), json!(4.0), json!(5.0), json!(6.0)]
    ) {
