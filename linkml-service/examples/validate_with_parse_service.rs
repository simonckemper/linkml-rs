...
use parse_core::{ParseFormat, ParseService, XmlFormat, JsonFormat, CsvFormat};
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let container_service = container_service::mock::create_mock_container_service()?;

    // Create task management service
    let task_service = Arc::new(task_management_service::factory::create_task_management_service()?);

    // Create cache service with all dependencies
    let cache_service = cache_service::factory::trait_object_factories::create_cache_service_trait_object(
        logger.clone(),
    ...
    let instances_path = PathBuf::from("/home/kempersc/apps/rootreal/domain/schema/place/polity/country/ISO3166Entity.yaml");
    let instances_content = std::fs::read_to_string(&instances_path)?;
    
    // Detect format (should be YAML)
    let (detected_format, confidence) = parse_service
        .detect_format(&instances_content)
        .await?;
    
    println!("Detected format: {:?} (confidence: {:.2}%)", detected_format, confidence * 100.0);
    
    // Parse the instance file
    let parsed_instances = parse_service
        .parse(&instances_content)
        .await?;
    ...
    println!("  - Validation time: {:?}", validation_result.validation_time);
    
    if !validation_result.errors.is_empty() {
        println!("\n❌ Validation Errors:");
        for error in &validation_result.errors {
            println!("  - [{}] {}: {}", error.severity, error.field, error.message);
        }
    }
...
"#, code);
        ...
        println!("  {} '{}': {} - {}", status_icon, code, status_text, description);
    ...
    let json_validation = parse_service
        .validate_document(&parsed_json, None)
        .await?;
