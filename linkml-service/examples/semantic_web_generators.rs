...
use linkml_service::generator::{
    ...
    ShExGenerator, ShExStyle,
    PlantUmlGenerator, PlantUmlDiagramType,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    entity.slots = vec!["id".to_string(), "name".to_string(), "created_date".to_string()];
    ...
    researcher.slots = vec!["orcid".to_string(), "affiliation".to_string(), "publications".to_string()];
    ...
    publication.slots = vec!["doi".to_string(), "title".to_string(), "authors".to_string(), 
                            "publication_date".to_string(), "journal".to_string(), "status".to_string()];
    schema.classes.insert("Publication".to_string(), publication);
    ...
    dataset.slots = vec!["description".to_string(), "size_mb".to_string(), "format".to_string(), 
                         "license".to_string(), "publications".to_string()];
    ...
    organization.slots = vec!["name".to_string(), "country".to_string(), "type".to_string()];
    schema.classes.insert("Organization".to_string(), organization);
    ...
    schema.slots.insert("created_date".to_string(), created_date_slot);
    ...
    schema.slots.insert("publications".to_string(), publications_slot);
    
    let mut affiliation_slot = SlotDefinition::default();
    affiliation_slot.range = Some("Organization".to_string());
    schema.slots.insert("affiliation".to_string(), affiliation_slot);
    ...
    for slot_name in &["description", "publication_date", "journal", "format", "country", "type"] {
    ...
    schema.enums.insert("PublicationStatus".to_string(), pub_status);
    ...
    let result = select_generator.generate(&schema, &GeneratorOptions::default()).await?;
    std::fs::write("research_select.sparql", &result[0].content)?;
    println!("Generated: research_select.sparql");
    println!("Sample query:\n{}\n...\n", result[0].content.lines().take(15).collect::<Vec<_>>().join("\n"));
    
    // CONSTRUCT queries
    println!("1.2 CONSTRUCT Queries:");
    let construct_generator = SparqlGenerator::new()
        .with_query_type(SparqlQueryType::Construct);
    let result = construct_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let ask_generator = SparqlGenerator::new()
        .with_query_type(SparqlQueryType::Ask);
    let result = ask_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let result = shexc_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let shexj_generator = ShExGenerator::new()
        .with_style(ShExStyle::Json);
    let result = shexj_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let result = class_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let er_generator = PlantUmlGenerator::new()
        .with_diagram_type(PlantUmlDiagramType::EntityRelationship);
    let result = er_generator.generate(&schema, &GeneratorOptions::default()).await?;
    ...
    let state_generator = PlantUmlGenerator::new()
        .with_diagram_type(PlantUmlDiagramType::State);
    let result = state_generator.generate(&schema, &GeneratorOptions::default()).await?;
    std::fs::write("research_states.puml", &result[0].content)?;
    println!("Generated: research_states.puml");
    
    // Mind map
    println!("3.4 Mind Map:");
    let mindmap_generator = PlantUmlGenerator::new()
        .with_diagram_type(PlantUmlDiagramType::MindMap);
    let result = mindmap_generator.generate(&schema, &GeneratorOptions::default()).await?;
