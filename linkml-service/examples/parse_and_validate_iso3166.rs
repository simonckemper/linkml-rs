...
impl CountryCodeValidator {
    ...
    fn parse_instances(&mut self, file_path: &PathBuf) -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        println!("  - Sample codes: {:?}", &valid_codes[..5.min(valid_codes.len())]);
    ...
    fn extract_field(&self, field: &str) -> Vec<String> {
        if let Some(ref result) = self.parse_result {
            match field {
                ...
                "label" | "name" => result.entities.iter()
                    .map(|e| e.label.clone())
                    .collect(),
                "tld" | "domain" => result.entities.iter()
        ...
        for (idx, address) in addresses.iter().enumerate() {
            if let Some(country) = address["country"].as_str() {
                ...
                if result.is_valid {
                    ...
                    println!("  Address {}: ✓ {} ({}) - {}", idx + 1, country, country_name, city);
                } else {
                    println!("  Address {}: ✗ {} - {} [INVALID COUNTRY CODE]", idx + 1, country, city);
                }
            }
        }
    }
