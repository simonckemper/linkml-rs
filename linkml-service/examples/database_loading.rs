...
use linkml_service::loader::{DatabaseLoader, DatabaseDumper, DatabaseOptions, ForeignKeyRelation, DataLoader, DataDumper};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    pg_options.table_mapping.insert("users".to_string(), "User".to_string());
    pg_options.table_mapping.insert("products".to_string(), "Product".to_string());
    pg_options.table_mapping.insert("orders".to_string(), "Order".to_string());
    pg_options.table_mapping.insert("order_items".to_string(), "OrderItem".to_string());
    
    // Define foreign key relationships
    pg_options.foreign_keys.insert("orders".to_string(), vec![
        ForeignKeyRelation {
    ...
    pg_options.foreign_keys.insert("order_items".to_string(), vec![
        ForeignKeyRelation {
    ...
    println!("PostgreSQL connection string: {}", pg_options.connection_string);
    ...
    mysql_options.column_mapping.insert("users".to_string(), user_columns);
    
    println!("MySQL connection string: {}", mysql_options.connection_string);
    ...
    let sqlite_options = DatabaseOptions {
        ...
        infer_types: true,
        include_tables: Some(["users", "products", "orders"].iter()
            .map(|s| s.to_string())
            .collect()),
        ..Default::default()
    };
    ...
    let mut complex_options = DatabaseOptions {
        ...
        exclude_tables: ["audit_log", "temp_data", "migrations"].iter()
            .map(|s| s.to_string())
            .collect(),
        batch_size: 5000,
        max_connections: 10,
    ...
    complex_options.primary_key_columns.insert(
    ...
    dump_options.table_mapping.insert("users".to_string(), "User".to_string());
    dump_options.table_mapping.insert("products".to_string(), "Product".to_string());
...
fn create_ecommerce_schema() -> SchemaDefinition {
    ...
    schema.classes.insert("OrderItem".to_string(), order_item_class);
