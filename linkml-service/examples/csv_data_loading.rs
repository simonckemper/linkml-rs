...
use linkml_service::loader::{CsvLoader, CsvDumper, CsvOptions, LoadOptions, DumpOptions};
...
"#;
    ...
    for (i, instance) in instances.iter().enumerate() {
        ...
        println!("  Temperature: {}°C", instance.data.get("temperature").unwrap());
...
"#;
    ...
    let mapped_instances = loader.load_string(csv_with_different_headers, &schema, &mapped_options).await?;
    println!("Loaded {} experiments with field mapping", mapped_instances.len());
...
";
    ...
    let tsv_instances = tsv_loader.load_string(tsv_data, &schema, &load_options).await?;
...
"#;
    ...
    let valid_instances = loader.load_string(invalid_csv, &schema, &strict_options).await?;
    println!("Loaded {} valid experiments (skipped invalid ones)", valid_instances.len());
    ...
    let csv_output = dumper.dump_string(&all_instances, &schema, &dump_options).await?;
    ...
    let limited_output = dumper.dump_string(&all_instances, &schema, &custom_dump_options).await?;
    ...
    // Dump and reload
    let roundtrip_csv = dumper.dump_string(&all_instances, &schema, &dump_options).await?;
    let reloaded = loader.load_string(&roundtrip_csv, &schema, &load_options).await?;
    
    println!("Original instances: {}", all_instances.len());
    println!("Reloaded instances: {}", reloaded.len());
    println!("Round-trip successful: {}", all_instances.len() == reloaded.len());
    ...
    dumper.dump_file(&all_instances, output_path, &schema, &dump_options).await?;
    println!("Saved to: {}", output_path.display());
    
    // Load from file
    let file_instances = loader.load_file(output_path, &schema, &load_options).await?;
...
fn create_research_schema() -> SchemaDefinition {
    ...
    schema.classes.insert("Experiment".to_string(), experiment_class);
