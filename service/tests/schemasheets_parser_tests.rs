...
async fn test_parse_enum_definition() {
    ...
    let schema = parser.parse_file(&test_file, Some("person_schema")).await.unwrap();

    // Verify enum was parsed
    assert!(schema.enums.contains_key("Status"), "Status enum should be present");
    ...
    // Verify enum values
    assert_eq!(status_enum.permissible_values.len(), 2, "Should have 2 permissible values");
}
...
async fn test_parse_enum_values() {
    ...
    let values: Vec<String> = status_enum.permissible_values.iter()
        .map(|pv| match pv {
    ...
    assert!(values.contains(&"ACTIVE".to_string()), "Should contain ACTIVE");
    assert!(values.contains(&"INACTIVE".to_string()), "Should contain INACTIVE");
}
...
async fn test_parse_type_definition() {
    ...
    assert!(schema.types.contains_key("EmailType"), "EmailType should be present");
    
    let email_type = &schema.types["EmailType"];
    assert_eq!(email_type.name, "EmailType");
    assert_eq!(email_type.description, Some("Email address type".to_string()));
    assert_eq!(email_type.base_type, Some("string".to_string()));
    assert!(email_type.pattern.is_some(), "Should have pattern constraint");
}
...
async fn test_parse_subset_definition() {
    ...
    assert!(schema.subsets.contains_key("required"), "required subset should be present");
    
    let required_subset = &schema.subsets["required"];
    assert_eq!(required_subset.name, "required");
    assert_eq!(required_subset.description, Some("Required fields".to_string()));
}
...
async fn test_parse_class_with_is_a() {
    ...
    assert!(schema.classes.contains_key("Employee"), "Employee class should be present");
...
async fn test_parse_slot_with_pattern() {
    ...
    assert!(person.attributes.contains_key("email"), "email attribute should be present");
    
    let email_slot = &person.attributes["email"];
    assert!(email_slot.pattern.is_some(), "email should have pattern constraint");
...
async fn test_parse_exact_mappings() {
    ...
    assert!(!person.exact_mappings.is_empty(), "Person should have exact mappings");
    assert!(person.exact_mappings.contains(&"schema:Person".to_string()), 
            "Should have schema:Person mapping");
    
    // Verify slot has exact mapping
    let name_slot = &person.attributes["name"];
    assert!(!name_slot.exact_mappings.is_empty(), "name should have exact mappings");
    assert!(name_slot.exact_mappings.contains(&"schema:name".to_string()),
...
async fn test_parse_all_mapping_types() {
    ...
    assert!(!person.exact_mappings.is_empty(), "Should have exact mappings");
    
    // Verify close mappings
    assert!(!person.close_mappings.is_empty(), "Should have close mappings");
    assert!(person.close_mappings.contains(&"foaf:Person".to_string()),
...
async fn test_parse_prefixes_sheet() {
    ...
    assert!(schema.prefixes.contains_key("schema"), "schema prefix should be present");
    assert!(schema.prefixes.contains_key("foaf"), "foaf prefix should be present");
    assert!(schema.prefixes.contains_key("skos"), "skos prefix should be present");
...
async fn test_parse_settings_sheet() {
    ...
    assert_eq!(schema.description, Some("A schema for person and employee entities".to_string()));
}
...
async fn test_parse_biolink_minimal() {
    ...
    let parser = SchemaSheetsParser::new();
    let schema = parser.parse_file(&test_file, Some("biolink_minimal")).await.unwrap();

    // Verify classes
    assert!(schema.classes.contains_key("NamedThing"), "NamedThing should be present");
    assert!(schema.classes.contains_key("BiologicalEntity"), "BiologicalEntity should be present");
    assert!(schema.classes.contains_key("Gene"), "Gene should be present");
...
async fn test_parse_api_models_with_subsets() {
    ...
    let parser = SchemaSheetsParser::new();
    let schema = parser.parse_file(&test_file, Some("api_models")).await.unwrap();

    // Verify subsets
    assert!(schema.subsets.contains_key("required"), "required subset should be present");
    assert!(schema.subsets.contains_key("public"), "public subset should be present");
    assert!(schema.subsets.contains_key("internal"), "internal subset should be present");
    
    // Verify class
    assert!(schema.classes.contains_key("ApiRequest"), "ApiRequest should be present");
}
