...
pub fn merge_slot_definitions(
    ...
    SlotDefinition {
        ...
        description: override_def.description.clone().or_else(|| base.description.clone()),
        ...
        pattern: override_def.pattern.clone().or_else(|| base.pattern.clone()),
        minimum_value: override_def.minimum_value.clone().or_else(|| base.minimum_value.clone()),
        maximum_value: override_def.maximum_value.clone().or_else(|| base.maximum_value.clone()),
...
pub fn topological_sort_classes(schema: &SchemaDefinition) -> Result<Vec<String>> {
    ...
    fn visit(
        ...
        if visiting.contains(name) {
            return Err(LinkMLError::other(format!("Circular inheritance detected at: {}", name)));
        }
...
mod tests {
    ...
    fn test_normalize_uri() {
        ...
        assert_eq!(normalize_uri("http://example.org/path#fragment"), "http://example.org/path");
        assert_eq!(normalize_uri(" http://example.org "), "http://example.org");
    }
