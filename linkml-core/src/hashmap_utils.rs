...
pub
type StringMap < V > = HashMap < Arc < str > ,
V > ;
///  Create a `StringMap` with pre-allocated capacity #[must_use]
pub
fn string_map_with_capacity < V > (capacity: usize) -> StringMap < V > {
...
#[must_use]
pub
fn intern_keys < V > (map: HashMap < String ,
V >) -> HashMap < Arc < str > ,
V > {
    use
    crate ::string_pool ::intern ;
    map . into_iter () . map (| (k ,
    v) | (intern (& k) ,
    v)) . collect ()
}
...
#[must_use]
pub
fn merge_arc_maps < V: Clone > (base : & HashMap < Arc < str > ,
V > ,
override_map : & HashMap < Arc < str > ,
V > ,) -> HashMap < Arc < str > ,
V > {
    let
    mut result = HashMap ::with_capacity (base . len () + override_map . len ()) ;
    for (k ,
    v) in base {
        result . insert (Arc ::clone (k) ,
        v . clone ()) ;
    }
    for (k ,
    v) in override_map {
...
pub
fn collect_keys_for_removal < K: Clone ,
V ,
F > (map : & HashMap < K ,
V > ,
predicate: F) -> Vec < K >
where F: Fn (& K , & V) -> bool , {
    map . iter () . filter_map (| (k ,
    v) | {
        if predicate (k ,
        v) {
            Some (k . clone ())
