...
impl SchemaSettings {
    ...
    pub fn set_custom<T: serde::Serialize>(&mut self, key: &str, value: T) -> Result<(), serde_json::Error> {
        self.custom.insert(key.to_string(), serde_json::to_value(value)?);
        Ok(())
    }
}
...
impl ImportSettings {
    ...
    pub fn get_resolution_strategy(&self) -> ImportResolutionStrategy {
        self.resolution_strategy.unwrap_or(ImportResolutionStrategy::Mixed)
    }
}
