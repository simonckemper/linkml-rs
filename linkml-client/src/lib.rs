...
where
    S: LinkMLService + Send + Sync + 'static,
{
    async fn load_schema(&self, path: &std::path::Path) -> Result<linkml_core::types::SchemaDefinition> {
        self.service.load_schema(path).await
    }
    
    async fn load_schema_str(&self, content: &str, format: linkml_core::traits::SchemaFormat) -> Result<linkml_core::types::SchemaDefinition> {
    ...
    async fn validate_typed<T>(&self, data: &serde_json::Value, schema: &linkml_core::types::SchemaDefinition, target_class: &str) -> Result<T>
    where
        T: serde::de::DeserializeOwned,
    {
        self.service.validate_typed(data, schema, target_class).await
    }
}